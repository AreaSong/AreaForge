...
blockers:
  - v1.1.1-source-checkpoint
  - current-bound-browser-compatibility-sc-002-sc-004
  - v1.1.1-release-admission-and-confirmation
risk: high
...
完成 `v1.1.0` 发布后的产品化修复、生产合成数据收口和 22 个主要入口复验，并形成不可覆盖既有 Release 的 `v1.1.1` 修复候选，再按新目标 commit 重新完成 Release admission。`v1.1.1` Release 与 production apply 分别确认，不自动关闭 residual。
...
- package version 已提升到 `1.1.1`；v1.1 专用 admission 与 Release workflow 已扩展为只接受 `v1.1.x` patch 系列并精确绑定 tag/version。当前仍需形成新的源码 checkpoint、重采证据，且尚未 push、创建 `v1.1.1` Release 或 production apply；Release 和生产 apply 仍分别确认，residual 状态不自动变化。
...
当前本地代码、完整迁移隔离库 M1-M3 runtime、生产清理、生产 22 入口复验和当前源码九入口桌面/移动快速验收已完成；`v1.1.1` 版本/admission/workflow 修改正在重新通过全量门禁。新源码 checkpoint/current-bound 正式 browser/compatibility、matching CI、SC-002/SC-004 和最终 Release admission 尚未完成；快速验收不冒充正式 evidence runner/validator。后续 push、受控 PR、tag、GitHub Release、production apply、backup/restore、migration deploy、updater apply/rollback 或 residual ledger update，均须按对应边界另行授权。
