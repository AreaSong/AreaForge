...
- 隔离浏览器矩阵发现并修复 member-only 账户登录后误进首次设置，以及成员只读 `/settings/exams` 仍调用 Owner/Admin 合并管理查询的问题；根入口现在把仅成员账户送到 `/settings/workspaces`，成员考试页只加载可读科目/分组。Owner/Admin/Coach/Member/Viewer 与 Platform Operator 的 1280×900/390×844 可见性、入口、只读/管理控件、跨 Workspace 404、无横向溢出和 console 0 error 已通过，证据见 `output/playwright/v15-role-matrix/evidence.json` 与同目录截图。代表性浏览器写入流程（角色、邀请、grant、Coach 建议、Operator 账户状态/会话）已在 `output/playwright/v15-write-matrix/evidence.json` 通过；完整失败变体、保护 PR/合并、Release 和生产证据仍待补齐。
