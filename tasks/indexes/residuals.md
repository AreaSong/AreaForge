...
| AF-RISK-DATA-002 | 多用户/Membership/RBAC 已有本地统一授权、敏感矩阵、跨租户负向、隔离 runtime/总门禁、脱敏 Operator 账户目录和六类身份 desktop/mobile 可见性矩阵；仍缺旧 nullable owner 收紧、浏览器写交互、受保护 PR/合并、Release 与生产证据，保持 deferred-work | `areaforge-security-governance` / `areaforge-validation-driver` | v1.4-v1.5 浏览器写交互/Release/生产证据齐全后 |
