...
状态：Package D 范围已完成。结构化模拟考试、阶段计划和本地规则阶段调整草稿已由 Package B Batch 5/6 完成；报告确认/驳回和只读回放已由 Package D Batch D1 完成；任务重排所选项确认/驳回/应用记录已由 Batch D2 完成；长期 AI 阶段草稿显式触发已由 Batch D3 完成；长期风险主题同源展示已由 Batch D4 完成；Package D D5 已完成证据收口。报告驱动的自动阶段应用不进入当前范围。
...
- 结构化模拟考试模型/API/UI 主路径已完成；阶段计划和阶段调整草稿持久化已由 Package B Batch 6 完成；报告决策入口已由 Package D Batch D1 完成；任务重排所选项确认/驳回/应用记录已由 Batch D2 完成；真实 AI 长期阶段草稿已由 Batch D3 完成。

## 后续单独确认的增强

- 结构化模拟结果已接入报告决策、任务重排确认流、长期阶段 AI 草稿和长期风险只读 DTO；若要继续做报告驱动自动阶段应用，必须另行确认。
- AI 长期阶段调整建议已接入显式草稿路径，长期风险已接入只读同源 DTO；继续推进时只能另行确认新的报告驱动阶段应用、AI 历史/费用账本或更大上下文字段。
- 长期 AI 阶段调整的隐私、费用、限流和可发送字段清单见 `tasks/backlog/0017-ai-stage-privacy-cost.md`；D3 已完成最小草稿路径，不能混入普通 AI 鞭策任务或页面普通 GET 中默认外呼。
...
- 阶段计划模型已存在，D3 已允许显式长期 AI 草稿；自动任务重排、批量改任务、报告驱动自动应用和 D3 范围外长期 AI 外呼不进入当前范围，未来需单独确认。
