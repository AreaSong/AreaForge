...
状态：进行中。Package D Batch D1 报告决策入口和 Batch D2 任务债务重排确认流已确认并完成；D3-D5 仍待后续确认。该任务承接 `docs/product/feature-scope.md` 的第二阶段增强，不替代已有 `0013` 和 `0014`。
...
- 该进展不代表第二阶段长期闭环完成；Package B Batch 2 已提供债务事件账本、Batch 5 已提供结构化模拟考试，Batch 6 已提供阶段计划和阶段调整草稿，Package D Batch D1 已提供报告决策入口，Batch D2 已提供债务重排确认/驳回/所选项应用；结构化复习历史、结构化长期风险、长期 AI 和阶段计划主题信号仍待 Package D 后续批次推进。
- `pnpm package-d:preflight` 已作为 Package D 批次感知门禁，聚合检查 D1-D5 确认句、completion record 锁定状态、报告/债务/阶段 route、未确认持久化禁区、长期 AI 禁区和 core 纯规则前置；该命令本身不写库、不新增 API、不解锁 Package D 主状态。D1 只有在完整证据存在时才狭窄放行报告决策入口，D2 只有在完整证据存在时才狭窄放行债务重排所选项写入口，D3-D5 继续锁定。

## 确认后实施切入点

以下清单用于 Package D 分批实现。D1 已完成；其余批次在确认前仍不得新增应用写 API、阶段计划应用路径、长期 AI 外呼或任务/阶段应用持久化。
