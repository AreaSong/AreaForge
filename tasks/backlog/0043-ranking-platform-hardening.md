...
- 本地候选已增加专用 `RankingAppeal` 持久表、唯一未决申诉约束、revision CAS、提交/查看/复核 API、正文不进入审计 metadata，以及成员邀请/接受/退出 UI；排名与投影具备独立默认关闭开关。尚未实现或收口：申诉 UI、通知、退出/删除/导出全链路联动、公开榜和生产动作。这些边界仍需 RANKING、AUTH/RBAC、DELETE/EXPORT 与 OPS 确认及运行证据。
- 当前工作树已形成本地候选 schema/migration、偏好 opt-in、私有挑战/参与者状态服务、可重建投影、专用持久申诉和删除预览 hook，以及 authenticated API routes；`RANKING_ENABLED` 与 `RANKING_PROJECTION_ENABLED` 默认关闭，migration 未 apply。全量 core/Web 门禁与隔离 PostgreSQL v1.8 runtime 已通过，尚缺申诉 UI、通知、删除/导出联动、完整浏览器矩阵和 Release/生产证据。
