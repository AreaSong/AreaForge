...
状态：Package E Batch E1 已完成；E2-E4 待确认，归属 Package E。当前已完成生产配置与发布工件预检、生产发布 runbook、compose/Nginx/Dockerfile 结构预检、发布记录草案和确认前护栏；生产部署、生产 migration deploy、真实备份恢复演练、发布后烟测和回滚记录仍需 Package E E2-E4 逐批确认。
...
| Batch E2 发布前备份与恢复演练 | PostgreSQL dump、上传目录归档、生产 `.env` 权限收紧备份、当前 compose/Nginx 副本、临时库导入、临时上传目录恢复、附件只读对账 | 记录 backup path/hash、`envBackupSha256`、`composeConfigBackupPath`、`nginxConfigBackupPath`；登录、首页、任务、计时、复盘可读；附件 metadata/hash 对账 `action=report_only` |
| Batch E3 生产发布与 migration deploy | 备份点校验、受控 release 工作目录或一次性 migration job、必要 additive migration deploy、compose/Nginx 切换、发布后烟测、日志脱敏检查 | `migrationRunner` 二选一并写入发布记录；`GET /api/health`、登录、首页、任务、计时、复盘、`/syllabus`、`/notes`、`/analytics`、`/reports`；附件和 AI 只用最小测试数据 |
| Batch E4 回滚演练与 Package E 收口 | 上一镜像 tag、回滚步骤、是否恢复数据库/上传目录、失败原因、恢复耗时、残余风险、文档同步和 completion record | 回滚记录可执行；填写 `rollbackPlan`、`rollbackDrillResult`、`rollbackDurationMinutes`、`databaseRestoreRequired`、`uploadsRestoreRequired` 和 `rollbackFailureReason`；`pnpm release:evidence:validate`、`pnpm docs:readiness`、`pnpm risk:preflight`、`pnpm docs:completion` 的 Package E 证据更新 |
