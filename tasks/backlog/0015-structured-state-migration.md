...
- 任务债务事件账本：补做、延期、放弃、拆小、改复习、完成动作和父子关系；重排确认、驳回和所选项应用记录已由 Package D Batch D2 完成，合并或批量自动应用仍需后续确认。
...
Package B 分批 migration 主线已完成，Package D Batch D1-D5 和 Package E 生产发布/远端签名更新也已完成。后续若要历史数据回填、删除旧字段、D3 范围外真实长期 AI、报告驱动自动任务/阶段应用或长期风险写入状态，必须继续走 `0017` 或新的单独高风险确认包。
...
以下清单记录 Batch 1 已实现范围；Batch 2、Batch 3、Batch 4、Batch 5 和 Batch 6 已在确认后完成，但未来仍不能绕过确认执行历史回填、删除旧字段、D3 范围外真实 AI 外呼、报告驱动自动应用或长期风险写状态。
