...
实现 Owner/Admin/Coach/Member/Viewer 权限矩阵、统一 policy service、角色分配/撤销、对象级分享 grant CRUD、敏感读取审计，以及成员授权下的 Coach 建议确认闭环。

## 必须先冻结

- 角色与 capability 矩阵、敏感数据矩阵、分享范围/到期/撤销语义、拒绝响应和 TOCTOU 策略。
