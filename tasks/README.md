...
- A -> B 平台演进的 M0 已归档到 `tasks/done/0038-platform-evolution-m0.md`，`tasks/active/0039-personal-dynamic-foundation.md` 正在收口 v1.3；后续按版本依次由 `0040`（v1.4 身份/Workspace/Membership）、`0044`（v1.5 RBAC/隐私/协作）、`0041`（v1.6 数据生命周期）、`0042`（v1.7 受控运维）、`0043`（v1.8 挑战排名）、`0045`（v1.9 平台加固）和 `0046`（v2.0 综合门禁）承接。任务进入 active 或总体路线获同意都不等于已经获得认证/权限、migration、生产写入或数据删除授权。
