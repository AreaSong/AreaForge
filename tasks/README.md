...
- A -> B 平台演进的 M0 已归档；v1.3 已合并但等待独立 Release/production disposition。`0040`、`0044`、`0041`、`0042`、`0043` 对应的 v1.4-v1.8 默认关闭本地候选已形成隔离 runtime、本地总门禁、Git 检查点和 branch push CI run `34021752169` 成功证据；仍待完整浏览器矩阵、受保护 PR/合并、Release 和生产证据。后续由 `0045`（v1.9 平台加固）和 `0046`（v2.0 综合门禁）承接。任务进入 active 或总体路线获同意都不等于已经获得生产 migration/apply、数据删除或 Release 授权。
