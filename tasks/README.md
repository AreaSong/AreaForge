...
- A -> B 平台演进的 M0 已归档；v1.3 已由 PR #56 合并但等待独立 Release/production disposition，v1.4-v1.9 默认关闭候选已由 PR #57 合并 `main` commit `b04ba98`。六类身份 desktop/mobile、代表性写入和 17 项失败矩阵已有隔离证据；`0045` 已记录 SEARCH 与 QUOTA（24/12 组）的独立本地专项，继续承接成员/存储及跨工作区总量配额、MFA/观测与跨域加固，`0046` 承接 v2.0 综合门禁。候选 migration 未 apply 到共享库或生产，仍缺新 Release/生产证据；任务进入 active 不等于执行授权。
