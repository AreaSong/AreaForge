...
当前进度：Package A-E 和 docs 100% 当前证据已完成；`backlog/` 中保留的跨批次任务是稳定入口或未来增强承接，不代表对应主线仍未完成。`done/` 中记录已完成的单点任务和 GitHub Release updater。当前远端 `v0.1.7` 发布与 update-agent/updater 状态见 `docs/development/release-v0.1.7-record.md`、`tasks/done/0018-github-release-updater.md` 和 `tasks/backlog/0014-deployment-backup-release.md`；`tasks/active/0019-update-request-expected-before-binding.md` 正在等待高风险本地实现确认，用于修复 Web 请求入队后生产状态变化的 stale-request/TOCTOU 风险。长期运营残余项以 `docs/development/residual-risk-ledger.md` 的 ID 为准，执行视图见 `tasks/indexes/residuals.md`。维护窗口历史投影见 `docs/development/maintenance-window-index.json`；它由源记录完整重建，不是 task index 或执行队列。当前 `v0.1.7` 长期证据快照仍为 `needs_live_evidence`，缺 post-update OPS-001 redacted smoke/status/evidence bundle/closure packet 和 `releaseEvidenceBundleHash` / release evidence backup hash；OPS-004 matching alert drill/preflight 已达到 `ready_for_human_close` 但未关闭 residual；维护或线程交接先用 `pnpm ops:handoff --summary` 查看只读摘要。若维护者形成 residual 复核结论，先用 `docs/development/residual-closure-review-template.md` 和 `pnpm residuals:closure:validate <record>` 固定记录；该记录不自动关闭台账。
