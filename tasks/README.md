...
- A -> B 平台演进的 M0 已归档到 `tasks/done/0038-platform-evolution-m0.md`，`tasks/active/0039-personal-dynamic-foundation.md` 正等待 v1.3 独立 Release/production disposition；`tasks/active/0040-multi-user-rbac.md` 的 v1.4 AUTH 与 `tasks/active/0044-rbac-privacy-collaboration.md` 的 v1.5 RBAC 已形成隔离 runtime、本地总门禁和桌面 spot check，v1.5-R 另有成员任务 owner migration、成员 PlanInbox 转任务 runtime selftest；`0041`、`0042`、`0043` 已形成默认关闭的本地候选和隔离 runtime 证据，但仍待完整浏览器矩阵、Git/PR/CI、Release 和生产证据。后续由 `0045`（v1.9 平台加固）和 `0046`（v2.0 综合门禁）承接。任务进入 active 或总体路线获同意都不等于已经获得后续权限包、生产 migration/apply、数据删除或 Release 授权。
