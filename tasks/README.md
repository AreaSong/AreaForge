...
- `0045` 的持久 worker 首批已形成新协议队列、租约代次、退避/死信、独立子进程恢复和受控排名通知处理器候选，并通过 50 条 migration 与 15 组内核隔离 runtime；通知候选的独立域级确认、排名重建、导出/删除处理器、共享/生产启用和 v2.0 门禁仍未完成。已合并的 PR #57 不再作为候选代码合并阻塞。
