...
- 重复科目按规范化名称、等价 stable key 或相同 legacy code 识别，页面显示全部业务引用、活动 session、考纲/模拟/知识点冲突、主知识点重分配数量和推荐保留科目。预览带执行前 snapshot hash；当前不提供合并写 endpoint，不自动迁移、不物理删除。
...
- 确认后的科目合并必须在单一事务中重新校验 snapshot hash，拒绝活动 session 和过期预览，显式处理唯一键冲突，迁移全部引用后仅软归档来源科目。
