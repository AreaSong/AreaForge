...
- 用户可以在节点上选择目标掌握等级，并勾选已完成的掌握条件；Batch 4 后条件会持久化到 `MasteryConditionRecord`。
...
Package B Batch 4 已在确认后新增 `MasteryConditionRecord`、`MasteryEvidence` 和 `MasteryRetest` additive migration，并接入条件、证据和复测写入路径。

本批仍不包含历史文本解析回填、删除旧字段、复测失败自动降级、生产 migration deploy、结构化模拟考试或阶段计划。
