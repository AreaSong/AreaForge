...
用于用户自行安排的同步自测和后续阶段模拟。

## 能力

- 创建结构化模拟考试并填写真实考试日期；可标记是否为第一次同步自测。
...
- 考试先以 `DRAFT` 保存和核对；启动专用计时后进入 `IN_PROGRESS`，仍可继续录入和收口；显式确认为 `CONFIRMED` 后，成绩与失分只读，补救候选才进入可执行阶段。
...
- 分科卷面满分必须由用户录入正数；旧记录缺少完整满分时显示“满分未录入”，趋势图不根据目标分或固定 100/500 分猜测满分。

## 字段

- `SimulationExam.name`：考试名称。
- `SimulationExam.examDate`：考试日期。
- `SimulationExam.isFirstSynchronized`：是否第一次同步自测。
...
用户按自己的目标考试日安排第一次同步自测。自测结果不是终点，而是后续阶段调整的输入；系统不内置某一年的固定考试节点。
...
- canonical 页面为 `/test/simulations`。列表优先继续最近未完成考试（`DRAFT` 或 `IN_PROGRESS`），再展示已确认历史和独立创建区；详情按录分、确认事实、补救入箱三步推进。
