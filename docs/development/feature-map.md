...
| `scope.rbac` | 复杂权限系统 | partial | `tasks/active/0044-rbac-privacy-collaboration.md`、`output/playwright/v15-role-matrix/evidence.json` | v1.5 预设角色、对象分享、Coach 协作、成员 PlanInbox 转 actor-owned 任务、脱敏 Operator 账户目录和六类身份 desktop/mobile 可见性矩阵已进入本地候选；旧 nullable owner 清理/收紧、浏览器写交互与 Release/生产证据仍缺 |
