...
2. 若请求声明了 `Content-Length`，先按 `MAX_UPLOAD_MB` 加 multipart 余量做上限预检，超限直接返回 `413`，不进入 body 解析；未声明长度时跳过预检，正文超限仍由后续 policy 校验兜底。
3. 使用 `request.formData()` 解析 form data，只接受一个 `file`。
4. 校验路径参数 `noteId` 存在；第一版不支持无归属附件。
5. 使用 `file.arrayBuffer()` 完整读取文件 bytes，再用 `MAX_UPLOAD_MB` 和
   `ALLOWED_UPLOAD_MIME` 构造 policy 并执行 size/MIME/magic-bytes 校验；这不是有界流式 parser。
6. 调用 `createAttachmentMetadataDraft`：
   - magic bytes 识别真实 MIME。
   - 拒绝空文件、超大文件、未知 magic bytes、声明 MIME 不一致。
   - 生成随机 `storedName`、sha256、`upload://attachment/...` URI。
7. 创建上传目录，解析安全路径，真实路径不能逃逸。
8. 使用独占写入模式写文件，避免覆盖同名文件。
9. 写入 `Attachment` metadata，并创建 `AuditEvent`。
10. 如果数据库写入失败，删除本次已写入文件。
11. 返回 `201 { attachment }`。
