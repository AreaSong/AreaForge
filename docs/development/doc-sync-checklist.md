...
- 新增、修改或移除已解决事故记录后，是否用 `pnpm incident:index` 完整重建 `docs/development/incident-index.json`，并通过 `pnpm incident:index:validate` 证明仅包含 `resolved + postIncidentReview=yes` 的记录、原始文件 hash、排序和 `latestIncidentId` 未漂移；索引是否仍明确不是 active incident 状态机、处置授权或 residual 关闭证据。
