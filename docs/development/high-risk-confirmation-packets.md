...
本确认包只覆盖当前 Workspace 内重复科目的显式合并写入。它不授权认证、Membership/RBAC、账户导出、物理删除、生产 migration、production apply 或服务器命令。

### 影响

- 将用户明确选择的来源科目全部业务引用迁移到一个保留科目：任务、学习 session、考纲节点、笔记、错题、模拟成绩、计划里程碑、计划收件箱、学习资料、知识点主/关联关系、知识分组和学习安排。
- 对考纲 `stableKey`、同一模拟考试的分科结果、知识点关联唯一键和主知识点关系做显式冲突处理。
- 来源科目只设置 `archivedAt` 软归档，保留历史记录；本批不物理删除科目、引用、附件或备份数据。
- 写入请求必须携带当前预览的 `snapshotHash`、目标科目、来源科目和用户确认意图；服务端重新读取并校验预览快照后才能进入事务。

### 风险

- 错误合并会改变历史统计、复习排期、模拟结果和知识点归属。
- 唯一键冲突、活动 session、并发编辑或预览过期可能造成部分迁移、重复关联或统计漂移。
- 事务外的附件、异步投影或审计写入失败可能留下不一致状态。
- 重放或篡改请求可能把同一来源科目重复迁移到不同目标科目。

### 实施范围

- 增加只接受结构化输入的 preview/confirm API；拒绝命令文本、自由 SQL、自由路径和客户端自报 workspace owner。
- confirm 前校验 actor 对 Workspace 的所有权、目标/来源科目均属于同一 Workspace、来源未归档、无活动 session、`snapshotHash` 与当前预览一致、请求幂等键未被其他范围使用。
- 在单一数据库事务内按固定顺序迁移各引用；对可合并的唯一关系使用确定性去重，对不可自动判断的冲突整体拒绝并返回冲突字段，不猜测用户意图。
- 成功后写入审计摘要、范围 hash、迁移计数和来源软归档结果；失败自动回滚，禁止“部分成功”。

### 验证

- 使用全新、明确命名的隔离 PostgreSQL fixture，验证 migration（如有）、事务原子性、唯一约束、活动 session 阻断和跨 Workspace 负向访问；不得连接生产或共享开发库。
- 覆盖无冲突合并、每类唯一冲突、过期 snapshot、并发 revision、重复幂等请求、篡改目标/来源、事务中断 kill-point、失败 rollback、前后各类引用计数和软归档状态。
- 预览输出与 confirm 实际执行范围逐字段比较；运行 core/Web 测试、typecheck、lint、`pnpm check`、API smoke，以及桌面/移动确认流程验收。
- 校验审计日志不包含附件路径、密钥、完整正文或内部数据库连接信息。

### 回滚

- 执行前保存范围 hash、各引用计数和来源/目标版本；单事务失败由数据库自动回滚。
- 成功后保留来源科目和迁移映射，提供限定时间的逆向撤销入口；撤销同样需要重新确认、幂等键和 snapshot 校验。
- 发现统计或投影异常时先停止后续合并、保留审计和映射，回滚应用代码；不物理删除、不直接改写历史数据。
- 生产 migration、备份、恢复和 production apply 必须另行提交对应确认包。
