...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | 独立确认的 EXPORT 已实现本人/Workspace 记录与 READY 附件私有流式 ZIP、权限版本重验、一次性 POST 下载及精确副本回收；90 个模型按 83 included/6 security/1 derived 分类，14 组运行态含四个进程强杀点及桌面/窄视口 API/浏览器验收通过。源数据与备份策略未改；仍缺数据库回收站、物理删除、源附件清理、删除账本持久化/恢复后真实重放及共享/生产交付，导出证据不关闭本残余项 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
