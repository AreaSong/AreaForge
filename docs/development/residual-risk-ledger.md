...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | 生命周期边界已于 2026-07-21 人工接受，允许隔离环境开放导入 confirm；v1.6 本地候选已增加 feature-gated 数据任务、脱敏预览、删除 preview 和一次性 grant descriptor，并在一次性隔离 fixture 验证；仍缺真实导出归档、物理删除、附件清理、备份副本同步删除与完整账户导出 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
