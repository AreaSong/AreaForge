...
| AF-RISK-REL-001 | accepted-exception | 2026-09-10 | AREAFORGE_AUTO_APPLY=none 是当前安全默认；patch 自动应用尚未启用 | 否 | 用户明确确认 patch 自动应用，且签名、备份、extra smoke、rollback target 和 manifest policy 同时满足 | 确认记录、updater env 摘要、release manifest、smoke 和 rollback evidence | `areaforge-release-operator` / `areaforge-sre-ops` |
...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | v1.6 本地候选已有数据任务、确定性内存 ZIP/真实字节 SHA-256、默认不可执行删除状态机、纯回收站协议和 deletion ledger/replay plan；账户/Workspace preview 范围已收紧，89 个 Prisma model 已全部分类，其中 83 个已接入 owner/actor-scoped preview、5 个安全排除、1 个派生重建，待最小化项已归零。静态门禁禁止选择 token/network hash、加密 API Key、nonce、幂等/请求指纹、objectKey、worker/lease 能力材料；隔离 runtime 已覆盖成员 session/挑战/参与/申诉与 Operator 自有受控请求。这些只证明纯规则和隔离 worker/descriptor 契约，仍缺真实导出临时存储/附件本体/下载流、数据库回收站、物理删除、附件清理、账本持久化/恢复后真实重放与完整账户导出 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
| AF-RISK-DATA-002 | deferred-work | 2026-11-15 | v1.4 AUTH 与 v1.5 RBAC 本地实施已确认并形成 additive migration、稳定资源 owner、统一授权层、五级角色/grant、跨租户负向、隔离 runtime、总门禁和 Git 检查点；commit `6568c87` 的 branch push CI run `34119581102` 成功。六类身份 desktop/mobile 可见性、脱敏 Operator 账户目录、跨 Workspace 404、代表性浏览器写入、App Shell/每日复盘 actor/owner 查询隔离和 17 项浏览器失败矩阵已通过，证据见 `output/playwright/v15-role-matrix/evidence.json`、`output/playwright/v15-write-matrix/evidence.json` 与 `output/playwright/v15-failure-matrix/evidence.json`。`StudyTask.ownerUserId` 已用 fail-closed migration 收紧为非空并通过 49-migration 隔离验证；本地候选仍缺受保护 PR/合并、签名 Release 和生产迁移证据，当前不能开放生产多人协作 | 是 | AUTH/RBAC 独立确认包、additive migration、旧 owner 回填、统一授权层、角色/敏感资源矩阵、跨租户负向测试、会话撤销和生产迁移证据齐全后进入人工关闭复核 | 威胁模型、migration preimage、权限矩阵、敏感数据矩阵、IDOR/API/service/query 测试、浏览器验收、审计记录、Release/production evidence | `areaforge-security-governance` / `areaforge-validation-driver` |
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | v1.8 本地候选已形成 scoreVersion、opt-in、挑战/参与者 CRUD、反作弊、投影、持久申诉、成员/Owner UI、持久排名通知、独立故障开关和删除预览；挑战、本人参与/申诉和通知已进入用户导出预览，隔离事务链证明成员 opt-out 后参与转 `LEFT`、投影归零、成员删除预览为 `READY`，未转移挑战的 Owner 保持 `BLOCKED`。申诉/通知另通过桌面/移动旅程与跨会话状态验证。仍缺真实物理删除/完整归档、完整跨租户浏览器矩阵和生产证据；直接排行仍可能泄露个人学习信息并激励刷时长 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
| AF-RISK-OPS-009 | deferred-work | 2026-11-30 | v1.7 本地候选已形成白名单 catalog、strict intent、持久请求/确认/审批/lease/hold/retry API，并在一次性隔离 fixture 验证；root-agent phase journal、跨进程锁、reconciliation、真实 updater/逐动作生产证据仍缺，扩大 Web 可请求的生产动作会增加提权、重放、状态漂移和误操作风险，Web runtime 任意 shell 仍永久禁止 | 否 | OPS 独立确认包、白名单 operation catalog、expected-before/TTL/nonce/hash/幂等/审批、root-only agent journal/锁/恢复以及逐动作生产证据齐全后进入人工关闭复核 | 协议与威胁模型、请求/agent selftest、kill-point、权限负向测试、签名/备份/hash/smoke/rollback/redacted evidence | `areaforge-sre-ops` / `areaforge-security-governance` / `areaforge-supply-chain` |
...
- `AF-RISK-DATA-002` -> `tasks/active/0040-multi-user-rbac.md`、`tasks/active/0044-rbac-privacy-collaboration.md`、`tasks/backlog/0045-platform-hardening.md`、`tasks/backlog/0046-v2-platform-gate.md`
