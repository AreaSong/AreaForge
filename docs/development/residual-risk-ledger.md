...
| AF-RISK-DATA-002 | deferred-work | 2026-11-15 | A -> B 已规划邀请制多用户、Membership 和 RBAC，但当前仍是单管理员 owner 语义；若未完成全链路 workspace scope、敏感数据矩阵和 IDOR 负向验证，开放成员能力会产生跨租户读写与隐私泄露风险 | 否 | AUTH/RBAC 独立确认包、additive migration、旧 owner 回填、统一授权层、角色/敏感资源矩阵、跨租户负向测试、会话撤销和生产迁移证据齐全后进入人工关闭复核 | 威胁模型、migration preimage、权限矩阵、敏感数据矩阵、IDOR/API/service/query 测试、浏览器验收、审计记录、Release/production evidence | `areaforge-security-governance` / `areaforge-validation-driver` |
