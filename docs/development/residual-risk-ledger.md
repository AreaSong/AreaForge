...
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | 私有挑战/计分/反作弊、opt-in、申诉与通知基础之外，RANKING 持久重建已在独立本地确认下通过 26 组运行态和 12 组浏览器/API 验收：严格权限/来源历史、代次拒绝、原子整榜、撤销/冻结/恢复/合成数据库删除、脱敏导出及两处强杀。复用 53 条 migration、无新增 DDL，默认关闭。完整版本/跨域旅程、受保护合并、Release 与生产证据仍缺；本次仅同步本地证据摘要，不改变分类、复核日期、可执行状态或关闭条件，排行仍需防范隐私泄漏和刷时长激励 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
