...
| AF-RISK-DATA-002 | deferred-work | 2026-11-15 | v1.4 AUTH 与 v1.5 RBAC 本地实施已确认并形成 additive migration、稳定资源 owner、统一授权层、五级角色/grant、跨租户负向、隔离 runtime、总门禁、Git 检查点和 branch push CI run `34021752169` 成功证据；完整浏览器矩阵、受保护 PR/合并、签名 Release 和生产迁移证据仍缺，当前不能开放生产多人协作 | 是 | AUTH/RBAC 独立确认包、additive migration、旧 owner 回填、统一授权层、角色/敏感资源矩阵、跨租户负向测试、会话撤销和生产迁移证据齐全后进入人工关闭复核 | 威胁模型、migration preimage、权限矩阵、敏感数据矩阵、IDOR/API/service/query 测试、浏览器验收、审计记录、Release/production evidence | `areaforge-security-governance` / `areaforge-validation-driver` |
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | v1.8 本地候选已形成 scoreVersion、opt-in、私有挑战/参与者 CRUD、反作弊规则、可重建投影、专用持久申诉、成员/Owner 申诉 UI、独立投影开关和删除预览，并通过隔离 fixture 与 390×844/1440×900 申诉浏览器旅程；仍缺通知、退出/删除/导出全链路联动、完整跨租户浏览器矩阵和生产证据；直接排行仍可能泄露个人学习信息并激励刷时长 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
