...
| AF-RISK-SC-002 | release-follow-up | 2026-07-24 | CI/Release 外部 GitHub Actions 已在本地 pin 到 40 位 commit SHA，`pnpm audit:prod` 已进入 workflow；仍缺 GitHub CI/Release 运行证据；`pnpm release:supply-chain:validate` 可校验下一次 run 证据记录 | 否 | 下一次 GitHub CI 或签名 Release 记录 Actions SHA pinning gate 和 `pnpm audit:prod` 输出，且无 high/critical 生产依赖漏洞 | workflow diff、`pnpm governance:preflight` 输出、`pnpm audit:prod` 输出、GitHub Actions run 记录或发布记录摘要、`pnpm release:supply-chain:validate` 通过 | `areaforge-supply-chain` / `areaforge-enterprise-governance` |
...
| AF-RISK-UX-001 | monitoring-gap | 2026-07-24 | 没有新鲜真实体验复核记录时，产品体验只能按功能 smoke 或历史截图判断，不能宣称桌面/移动体验完整健康 | 是 | 完成一次覆盖 desktop 和 mobile 的产品体验复核记录，包含本地或生产 smoke、浏览器观察或截图、关键旅程、确认边界和安全事实，并通过 `pnpm experience:review:validate` | `pnpm smoke:local-ux` 或生产只读 smoke 输出、desktop/mobile 截图或浏览器记录、`pnpm experience:review:validate` 通过、后续优化项摘要 | `areaforge-product-experience` / `areaforge-qa-smoke` |
