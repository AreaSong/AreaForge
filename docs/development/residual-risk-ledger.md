...
| AF-RISK-OPS-009 | deferred-work | 2026-11-30 | OPS 独立本地确认已完成冻结绑定、root-owned 桥接登记/文件、跨进程锁、不可覆盖日志、停止屏障与回执恢复；38 组合成运行态和真实 API/桌面/390px/320px 验收通过。生产适配器未运行，签名 Release、生产配置/逐动作 backup/apply/rollback 与运营证据仍缺；Web 任意 shell 永久禁止。本次仅同步本地证据摘要，不改变 residual 分类、复核日期、可执行状态或关闭条件 | 否 | OPS 独立确认包、白名单 operation catalog、expected-before/TTL/nonce/hash/幂等/审批、root-only agent journal/锁/恢复以及逐动作生产证据齐全后进入人工关闭复核 | 协议与威胁模型、请求/agent selftest、kill-point、权限负向测试、签名/备份/hash/smoke/rollback/redacted evidence | `areaforge-sre-ops` / `areaforge-security-governance` / `areaforge-supply-chain` |
