...
| AF-RISK-OPS-006 | current-blocker | 2026-07-22 | canonical partial unique index、session/task CAS、结束计时单次副作用和 CheckIn advisory lock 已在当前 checkout 本地实现；隔离 PostgreSQL migration/并发 runtime record 与 doctor before/after 已通过，strict preflight 返回 `local_verified`。该实现尚未进入匹配签名 Release 或生产 migration/deploy，线上 `v0.1.7` 仍不能据此宣称具备该并发保护 | 否 | 保留当前 `local_verified` 证据；创建匹配 exact commit 的签名 Release并通过 release/supply-chain 门禁；随后另行确认生产 migration/deploy，先取 fresh production doctor、应用 additive index/匹配应用、验证 health/登录态 smoke/409 与单次副作用，再取 after doctor 和回滚证据；最后由维护者人工复核关闭 | `docs/development/ops-006-business-state-concurrency-design.md`、`tasks/active/0020-business-state-concurrency.md`、`prisma/migrations/20260718010000_add_active_session_unique_index/migration.sql`、`output/ops006/concurrency-runtime-20260718.json`、`output/ops006/data-integrity-before-20260718.json`、`output/ops006/data-integrity-after-20260718.json`、`pnpm ops:ops-006:runtime:validate` 与 strict preflight 通过、`pnpm db:validate`、`pnpm check`、匹配签名 Release、独立生产 migration/deploy 记录、fresh production before/after doctor、health/authenticated smoke、409/单次副作用证据和 rollback target | `areaforge-security-governance` / `areaforge-sre-ops` / `areaforge-validation-driver` |
