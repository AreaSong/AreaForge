...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | v1.6 本地候选已有 feature-gated 数据任务、脱敏 preview、一次性 grant descriptor、确定性内存 ZIP/真实字节 SHA-256，以及默认不可执行的删除状态机；这些只证明纯规则和隔离 worker 契约，仍缺真实导出临时存储/附件本体/下载流、物理删除、附件清理、备份 deletion ledger 与完整账户导出 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
...
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | v1.8 本地候选已形成 scoreVersion、opt-in、私有挑战/参与者 CRUD、可重建投影和删除预览，并在一次性隔离 fixture 验证；仍缺完整反作弊、申诉、通知、故障总开关、退出/删除/导出联动和生产证据；直接排行仍可能泄露个人学习信息并激励刷时长 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
