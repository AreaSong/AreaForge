...
| AF-RISK-SC-001 | deferred-work | 当前 Release workflow 已接入 SBOM/provenance 生成路径；线上 `v0.1.5` 仍是无 SBOM/provenance 的历史签名发布 | 否 | 下一次签名 Release 生成并发布 SBOM/provenance，updater 或验证脚本校验 checksum/signature，并在发布记录中留下证据 | SBOM asset、provenance asset、`SHA256SUMS` 覆盖记录、签名校验输出、发布记录摘要 | `areaforge-supply-chain` |
| AF-RISK-SC-002 | release-follow-up | CI/Release 外部 GitHub Actions 已在本地 pin 到 40 位 commit SHA，`pnpm audit:prod` 已进入 workflow；仍缺 GitHub CI/Release 运行证据 | 否 | 下一次 GitHub CI 或签名 Release 记录 Actions SHA pinning gate 和 `pnpm audit:prod` 输出，且无 high/critical 生产依赖漏洞 | workflow diff、`pnpm governance:preflight` 输出、`pnpm audit:prod` 输出、GitHub Actions run 记录或发布记录摘要 | `areaforge-supply-chain` / `areaforge-enterprise-governance` |
| AF-RISK-SC-003 | deferred-work | 本地和历史 smoke 仍出现 `pg@9` 未来弃用 warning；当前 `pg@8` 通过，但升级前需要定位 Prisma adapter 或并发查询来源 | 否 | `pg` / `@prisma/adapter-pg` 升级前用 `--trace-deprecation` 或等价测试定位来源，并证明核心 smoke 无该 warning 或有明确上游风险接受记录 | trace 日志、依赖升级验证、`pnpm smoke:local-ux` 或等价 smoke 输出、关闭或接受记录 | `areaforge-supply-chain` / `areaforge-sre-ops` |
