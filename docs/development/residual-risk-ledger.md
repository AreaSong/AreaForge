...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | 生命周期边界已于 2026-07-21 人工接受，允许隔离环境开放导入 confirm；已确认导入的规范化 Markdown 仍长期留存并随数据库备份扩散；v1.1 仅软归档与一次性 canonical 导出，仍缺物理删除、备份副本同步删除与完整账户导出 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
| AF-RISK-DATA-002 | deferred-work | 2026-11-15 | v1.4 AUTH 本地实施已确认，身份、Membership、Workspace 生命周期、默认关闭开关和 owner-only 学习正文边界已形成并通过隔离 runtime；最终浏览器/总门禁、v1.5 RBAC、签名 Release 和生产迁移证据仍缺，当前不能开放生产多人协作 | 是 | AUTH/RBAC 独立确认包、additive migration、旧 owner 回填、统一授权层、角色/敏感资源矩阵、跨租户负向测试、会话撤销和生产迁移证据齐全后进入人工关闭复核 | 威胁模型、migration preimage、权限矩阵、敏感数据矩阵、IDOR/API/service/query 测试、浏览器验收、审计记录、Release/production evidence | `areaforge-security-governance` / `areaforge-validation-driver` |
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | 私有挑战与排名已进入 v1.8 计划，但尚无指标版本、opt-in、异常/作弊、申诉、退出和删除联动；直接排行可能泄露个人学习信息并激励刷时长 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
| AF-RISK-OPS-009 | deferred-work | 2026-11-30 | 受控运维中心已进入 v1.7 计划；扩大 Web 可请求的生产动作会增加提权、重放、状态漂移和误操作风险，Web runtime 任意 shell 仍永久禁止 | 否 | OPS 独立确认包、白名单 operation catalog、expected-before/TTL/nonce/hash/幂等/审批、root-only agent journal/锁/恢复以及逐动作生产证据齐全后进入人工关闭复核 | 协议与威胁模型、请求/agent selftest、kill-point、权限负向测试、签名/备份/hash/smoke/rollback/redacted evidence | `areaforge-sre-ops` / `areaforge-security-governance` / `areaforge-supply-chain` |
...
- `AF-RISK-DATA-002` -> `tasks/active/0040-multi-user-rbac.md`、`tasks/backlog/0044-rbac-privacy-collaboration.md`、`tasks/backlog/0045-platform-hardening.md`、`tasks/backlog/0046-v2-platform-gate.md`
