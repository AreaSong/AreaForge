...
|---|---|---|---|---|---|---|---|
| AF-RISK-OPS-001 | monitoring-gap | 2026-07-17 | 生产 extra smoke 仍依赖服务器配置；没有新鲜 smoke 记录时，发布体验验证只能到 `warn` | 是 | 服务器配置 `AREAFORGE_EXTRA_SMOKE_COMMAND`、smoke 密码文件和最近一次通过记录 | redacted updater env 摘要、`pnpm smoke:prod-readonly` 通过时间、update-record 摘要 | `areaforge-sre-ops` / `areaforge-qa-smoke` |
| AF-RISK-OPS-002 | deferred-work | 2026-08-10 | 写入型生产 smoke 策略未定义；默认不污染生产业务数据 | 否 | 明确 smoke 账号、数据清理策略、允许写入范围、失败回滚方式并获确认 | 策略文档、测试账号、写入和清理 smoke 记录 | `areaforge-qa-smoke` / `areaforge-security-governance` |
...
| AF-RISK-SC-003 | deferred-work | 2026-08-10 | 本地和历史 smoke 仍出现 `pg@9` 未来弃用 warning；当前 `pg@8` 通过，但升级前需要定位 Prisma adapter 或并发查询来源 | 否 | `pg` / `@prisma/adapter-pg` 升级前用 `--trace-deprecation` 或等价测试定位来源，并证明核心 smoke 无该 warning 或有明确上游风险接受记录 | trace 日志、依赖升级验证、`pnpm smoke:local-ux` 或等价 smoke 输出、关闭或接受记录 | `areaforge-supply-chain` / `areaforge-sre-ops` |
...
| AF-RISK-OPS-004 | monitoring-gap | 2026-08-10 | 结构化 metrics dashboard 和告警路由不是当前产品功能；健康判断依赖 runbook 和只读证据 | 否 | 建立外部告警接收人、阈值、演练记录，或明确接受人工巡检窗口 | 告警配置摘要、演练记录、阈值文档 | `areaforge-observability` |
