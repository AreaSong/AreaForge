...
| AF-RISK-DATA-001 | deferred-work | 2026-10-21 | EXPORT 的私有 ZIP、权限重验、一次性下载与副本回收已有独立本地验收；新增六个删除协议模型后清单为 96 个模型（83 included/12 security/1 derived）。独立 DELETE 已实现持久回收站/冻结、物理文件与数据库删除、最小账本及备份水位重放，53-migration 专用合成库通过 17 组运行态和当前源码 API/桌面/390px/320px 验收。共享/生产源数据及备份策略未改；签名 Release、生产交付、生产恢复与全域联动证据仍缺。本次仅同步证据摘要，不改变 residual 类型、复核日期、可执行状态或关闭条件，不关闭本残余项 | 否 | 关闭：owner 鉴权/软归档/backup-restore/一次性导出临时资源释放 fixture 与 redaction matrix 齐备，且未来物理删除/撤销路线有单独确认包与实现证据后，方可人工关账。重开：缩短保留期、物理删除、备份副本同步删除、完整账户导出、扩大到 AI history、跨用户可见、或 confirm 落库策略变化 | docs/development/high-risk-confirmation-packets.md（learning-tree data lifecycle 已确认）、导入鉴权/归档/backup-restore/export fixture、redaction matrix、residual close/reopen condition | `areaforge-security-governance` / `areaforge-file-storage-safety` |
...
| AF-RISK-DATA-003 | deferred-work | 2026-12-15 | v1.8 本地候选已形成 scoreVersion、opt-in、挑战/参与者 CRUD、反作弊、投影、持久申诉、成员/Owner UI、持久排名通知、独立故障开关和删除预览；挑战、本人参与/申诉和通知已进入用户导出预览，隔离事务链证明成员 opt-out 后参与转 LEFT、投影归零、成员删除预览为 READY，未转移挑战的 Owner 保持 BLOCKED。申诉/通知另通过桌面/移动旅程与跨会话状态验证。完整归档已有独立本地 EXPORT 证据；仍缺真实物理删除、完整跨租户浏览器矩阵和生产证据；直接排行仍可能泄露个人学习信息并激励刷时长 | 否 | RANKING 独立确认包、个人基准、可重建投影、隐私字段禁区、反作弊、退出/删除重建和申诉证据齐全后进入人工关闭复核 | 指标/隐私规范、scoreVersion fixture、重复 session/异常/删除重建测试、挑战 opt-in/退出浏览器验收、跨租户负向测试 | `areaforge-product-experience` / `areaforge-security-governance` / `areaforge-validation-driver` |
