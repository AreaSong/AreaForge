...
本文件记录 AreaForge 进入真实业务代码前的闭环标准。当前前期治理、任务路线和 MVP 前工程准备已经完成，业务代码也已推进到低风险闭环阶段；因此这里不再作为当前业务完成度判断来源，当前进度以 `docs/development/implementation-order.md`、`tasks/**` 和实际代码验证为准。
...
这些事项在开发前闭环阶段不直接实现，后续按 `docs/development/implementation-order.md` 和 `workflow/versions/v0.1-mvp.md` 分批推进。

其中认证、业务 migration、上传、AI 边界变化和部署发布仍属于高风险任务，必须按影响、风险、验证和回滚确认后执行。
