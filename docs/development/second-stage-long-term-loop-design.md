...
| Package B Batch 4 未完成 | 基于任务、计时、笔记、错题更新时间派生遗忘风险 | 写入显式掌握证据、复测记录或结构化复习历史 |
| Package B Batch 5 未完成 | 只读兼容 `StudyTask.type = "simulation_exam"` 的模拟记录 | 写入结构化 `SimulationExam` 或科目结果 |
| Package B Batch 6 未完成 | 生成本地规则阶段草稿并标记需要用户确认 | 保存或应用 `StagePlan` / `StageAdjustmentDraft` |
| Package C 未完成 | 使用本地规则，不外呼长期 AI | 发送长期数据、动机档案、复盘正文、附件内容或阶段调整上下文给 provider |
