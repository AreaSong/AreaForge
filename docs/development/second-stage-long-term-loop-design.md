...
本文件是 Package D 的分批确认设计，不代表第二阶段长期闭环已经完成。Package D Batch D1 报告决策入口已完成；任何会应用任务重排、外呼长期 AI、自动应用模拟考试后的阶段调整或改变长期状态口径的后续实现，都必须等用户明确确认对应批次后再做。
...
- 恢复模式已有最小候选任务规则。
- 模拟考试已有结构化主路径；阶段计划和阶段调整草稿已有持久模型；报告决策入口已由 D1 完成，但长期 AI、任务重排应用和报告驱动的任务/阶段应用仍未实现。
...
| Package B Batch 4 已完成但 Package D D4 未确认 | 读取显式掌握证据和复测记录，并与任务、计时、笔记、错题更新时间一起派生遗忘风险 | 写入结构化复习历史、长期风险应用记录 |
...
- 任务债务重排的“应用”按钮或写 API。
- 阶段调整草稿的长期 AI 生成或扩展应用写路径。
- 周/月报告驱动的任务/阶段应用写 API。
- 长期 AI 阶段调整外呼。
- 结构化模拟考试后的阶段计划自动应用路径。
- 任务重排应用记录、阶段计划应用记录或长期 AI 建议持久化。
...
- 未确认批次边界烟测：不存在重排应用写 API；长期 AI 外呼关闭；建议 DTO 均为 `canAutoApply=false` 和 `requiresUserConfirmation=true`。
