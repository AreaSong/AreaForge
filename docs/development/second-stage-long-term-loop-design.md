...
这些能力已构成当前长期闭环主线；报告驱动自动阶段应用、批量改任务、长期风险写入状态、长期 AI 历史保存和费用账本仍不进入当前范围。
...
- Package B 已完成；若未来在缺失结构化状态的环境运行，只能提供只读派生建议，不允许写入应用记录。
...
| Package B Batch 5 已完成、Package D 对应批次完成前 | 读取和写入结构化 `SimulationExam` / `SimulationSubjectResult`，旧 `StudyTask.type = "simulation_exam"` 只读兼容 | 自动迁移旧任务型模拟、把模拟结果自动应用到阶段计划 |
| Package B Batch 6 已完成、Package D 对应批次完成前 | 保存阶段计划和本地规则阶段草稿；用户可显式确认草稿更新关联 `StagePlan` 并写审计；D1 可记录报告确认/驳回和回放 | 自动任务重排、报告驱动的任务/阶段应用、批量改任务、长期 AI 外呼或保存应用记录 |
...
- 不把 D2 的应用记录单独作为 Package D 完成证据；历史执行时 D2 后仍需 D3-D5，当前 D3-D5 已完成，生产发布由 Package E 独立完成。
...
- `pnpm check`、`pnpm risk:preflight`、`pnpm docs:readiness` 通过；Package E 和远端签名 Release 收口后，`pnpm docs:completion` 也应通过并持续作为回归门禁。
