...
| Package B | NOT_READY / 未完成 | 结构化 migration 设计和 Batch 0-6 已写入 `docs/development/structured-state-migration-design.md`；用户已确认并完成 Batch 0 `StudySession` 结构化收口字段，临时库 deploy、API 烟测和首页刷新烟测通过；Batch 1-6 确认包和专项验证矩阵已补齐；Batch 1 的逐日快照、幂等刷新、active session 展示和缺失日期 fallback 契约已同步到 `tasks/backlog/0015-structured-state-migration.md` 与 `docs/modules/check-in.md`；`packages/core` 已提供并测试 `buildDailyCheckInSnapshot` 纯规则，覆盖快照字段来源和低转化 fallback；dashboard、analytics 和 reports 的旧数据派生日级口径已复用该纯规则；`pnpm risk:preflight` 检查确认前边界，并扫描 schema 与 migration SQL 阻止 Batch 1-6 未确认结构提前出现 | 缺 Batch 1-6 additive migration、完整结构化读写路径和整包页面/API 烟测 | 等用户单独确认 Batch 1：`CheckIn` 日快照 |
...
- Package B：Batch 1 确认前准备进一步补强：`buildDailyCheckInSnapshot` 新增测试覆盖显式 `isLowConversion=false` 覆盖历史 `isEffective=false` fallback；Batch 1 确认包和 CheckIn 模块补充周/月 `taskCompletionRate` 默认采用逐日快照平均值，后续若要任务数加权需读任务明细或另行确认字段。
...
- Package A-E 均未达到完成证据要求；其中 Package B 只完成 Batch 0，Batch 1-6 仍待逐批确认和验证。

因此当前准确进度是：治理、确认包、Batch 0 和大量低风险基础能力已完成；docs 100% 尚未完成，下一条实现主线应从 Package B Batch 1 `CheckIn` 日快照开始。

## 最近验证

- `pnpm docs:readiness`：通过，说明治理结构和追踪入口存在。
- `pnpm docs:completion`：预期失败，当前阻塞为第一版的附件上传、真实 AI 建议，第二阶段的完整模拟考试、长期 AI 阶段调整，Package B Batch 1-6 批次证据，以及 Package A-E 完成证据。
