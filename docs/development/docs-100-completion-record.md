...
| Package B | NOT_READY / 未完成 | 结构化 migration 设计和 Batch 0-6 已写入 `docs/development/structured-state-migration-design.md`；用户已确认并完成 Batch 0 `StudySession` 结构化收口字段、Batch 1 `CheckIn` 日快照和 Batch 2 债务事件与父子任务；Batch 2 新增 additive `StudyTask.parentTaskId` 和 `TaskDebtEvent` migration，债务任务动作继续写 `AuditEvent` 并同步写入事件账本，拆小任务写入父子关系，旧任务继续按 `status/debtStatus/plannedDate` fallback；临时库 migration deploy、服务级烟测、`pnpm check`、文档同步和残余风险记录见下方批次证据 | 缺 Batch 3-6 additive migration、恢复状态、掌握证明显式证据、结构化模拟考试、阶段计划与整包页面/API 烟测 | 下一步等待用户单独确认 Batch 3：`RecoveryState` 恢复状态 |
...
- Package B：Batch 2 已在确认后完成 `StudyTask.parentTaskId` 与 `TaskDebtEvent`；债务任务动作双写现有 `AuditEvent` 和新事件账本，拆小任务写父子关系，旧任务继续按状态字段 fallback；`GET /api/tasks/debt-reorder` 仍保持只读，不写 `reorder_suggested/reorder_applied`，后续应用记录仍归 Package D。
...
- Package A-E 均未达到完成证据要求；其中 Package B 已完成 Batch 0-2，Batch 3-6 仍待逐批确认和验证。

因此当前准确进度是：治理、确认包、Batch 0-2 和大量低风险基础能力已完成；docs 100% 尚未完成，下一条实现主线应从 Package B Batch 3 `RecoveryState` 开始。

## 最近验证

- `pnpm docs:readiness`：通过，说明治理结构和追踪入口存在。
- `pnpm docs:completion`：预期失败，当前阻塞为第一版的附件上传、真实 AI 建议，第二阶段的完整模拟考试、长期 AI 阶段调整，Package B Batch 3-6 批次证据，以及 Package A-E 完成证据。
- `pnpm check`：通过，说明当前工程基线可构建；本次覆盖 Batch 2 新增 Prisma 客户端生成、全 workspace typecheck、Web lint、Prisma validate 和 Next build。
