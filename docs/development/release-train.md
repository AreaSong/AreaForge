...
`v0.1.7` 已生成并校验签名 Release 供应链证据，并已通过服务器侧 updater 应用到生产。下一次 Release 仍必须在确认句中写明具体版本；签名 Release 证据闭环确认包只覆盖 tag、GitHub Release workflow 和供应链记录，不覆盖服务器 updater apply、生产备份、migration、rollback 或 residual 台账关闭，生产 apply 仍需单独确认。
...
6. 发布 admission 必须确认 tag commit 已进入 GitHub 默认分支；Release workflow 按 repository + tag 串行化，不能让同一版本的两个发布运行交错写入 GHCR 或 GitHub Release。
7. 不可变身份探测必须三态 fail closed：只有明确不存在才能继续；Release/GHCR 已存在或 GitHub、GHCR、网络、权限、限流探测失败都必须停止。
8. 提交 checkpoint：提交信息说明用户可见或治理可见效果。
9. 创建并推送 tag：`vX.Y.Z`。
10. 等待 GitHub Release workflow：validate job 必须先通过，stable signing 缺 key 必须 fail closed；`release:workflow:policy` 锁定完整 workflow 的受审查 SHA-256，任何 YAML、job、step、shell 或注释变化都必须显式更新策略摘要并重跑 mutation selftest。
11. 验证 Release assets：manifest、SBOM、provenance、`SHA256SUMS`、`SHA256SUMS.sig`、`docker-compose.prod.yml`。
12. 记录供应链证据：需要关闭或复核 `AF-RISK-SC-001` / `AF-RISK-SC-002` 时，填写供应链记录并运行校验。
13. 触发受控更新：通过 Web 版本中心提交请求，或由管理员执行服务器侧 updater。
14. 记录生产证据：health、update-agent、smoke、backup、migration、rollback target、`ops:evidence:bundle` hash、告警预览和 residual risk IDs。
15. 若生产证据之后需要提交 UX、redacted evidence 或 residual 状态，按 `docs/development/release-evidence-closeout-contract.md` 形成 evidence-only closeout，运行 `pnpm release:closeout:binding:selftest`；source path 漂移必须回到新的 Release 流程。
...
发布前只运行模板和 validator selftest；此时尚无签名 Release assets，不能把 record-only 草稿校验当成供应链就绪。Release assets 生成后，按“供应链记录”一节同时提供 record/assets 并 strict 校验。

若本次 release/update 之后要声明“产品可长期运营”，必须在 OPS-001、OPS-004、OPS-005、OPS-006、与 OPS-006 after-doctor 绑定的数据完整性记录、签名 Release 供应链、完整 Release evidence 和新鲜 UX 证据都已保存后运行：
...
Release/update 后若需要固定当前证据和缺口，保存并校验只读长期证据快照：
...
生产 backup、migration、smoke、附件 reconciliation 和 rollback 证据全部齐备后，才运行完整发布记录校验：
...
- `pnpm ops:long-term:snapshot` 的 `snapshotHash`、状态和缺失证据摘要。
...
如果 Release 资产已下载到本地目录，可先用 `pnpm release:supply-chain:record <release-assets-dir>` 生成记录草稿；生成器会核对 `SHA256SUMS` 与 manifest/SBOM/provenance/compose 实物 hash 是否一致，仍要求显式填写 GitHub workflow run URL、validate job、`pnpm audit:prod`、governance、Actions pinning、checksum 和签名校验结果，不会连接 GitHub 或创建 Release。record-only 校验仅证明草稿结构；签名 Release 就绪必须运行 `pnpm release:supply-chain:validate <record> <release-assets-dir> --strict`。

如果只是用 GitHub CI 运行复核 `AF-RISK-SC-002`，使用 `docs/development/ci-supply-chain-record-template.md` 和 `pnpm ci:supply-chain:validate <record>`；CI-only 记录必须证明 `expectedGitCommit` 和 GitHub run 的 `gitCommit` 一致，不包含 Release SBOM/provenance、checksum 或 signature，因此不能关闭 `AF-RISK-SC-001`。记录保存到本地后可运行 `pnpm sc:sc-002:preflight`：CI-only 路径只读取 `AREAFORGE_SC002_CI_RECORD`；签名 Release 路径必须同时读取 `AREAFORGE_SC002_RELEASE_RECORD` 与 `AREAFORGE_SC002_RELEASE_ASSETS_DIR` 并 strict 校验 assets/checksum/cosign。该 preflight 不连接 GitHub、不创建 Release、不推 tag。
...
- residual ledger 中存在未被本次 Release admission 逐项标记 disposition、owner、允许阶段和停止条件的 current release blocker。`release-target` 只允许创建用于修复该 residual 的匹配签名 Release，不授权 production apply、人工关闭或完成声明。
...
- `AF-RISK-SC-001`：`v0.1.7` 签名 Release 已有 SBOM/provenance 资产、校验记录和生产 apply 记录，可进入人工关闭复核；生产 apply 不自动关闭 residual，后续 Release workflow、签名策略或新 Release 变更前需重新复核。
- `AF-RISK-SC-002`：已关闭为 CI-only 证据项；后续 GitHub CI/Release 运行仍需以 Actions pinning 和 `pnpm audit:prod` 证据复核。先跑 `pnpm sc:sc-002:preflight`，CI-only 走 `pnpm ci:supply-chain:validate`，签名 Release 走 `pnpm release:supply-chain:validate <record> <release-assets-dir> --strict`。
- `AF-RISK-OPS-004`：告警预览不等于真实外部告警，演练记录另行校验。
- `AF-RISK-OPS-005`：`v0.1.8` 可在 admission 中标记为 `release-target`，范围只到创建并严格校验匹配的签名资产；签名完成后仍是 production blocker，不授权 timer/队列/agent/Web 变更、请求执行或 residual 关闭。
- `AF-RISK-OPS-006`：本地 `local_verified` 可进入 matching Release，签名资产不授权 production migration/deploy；fresh before/after doctor、canonical index 读回、health/authenticated smoke、单独确认的 synthetic 409/单次副作用 probe、rollback target 和人工关闭仍是独立门禁。
