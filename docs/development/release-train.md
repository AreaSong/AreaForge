...
若下一次 Release 的目标是补齐 `AF-RISK-SC-001` 的 SBOM/provenance 签名证据，先使用 `docs/development/high-risk-confirmation-packets.md` 中“后续签名 Release 证据闭环确认包”。当前 `v0.1.5` 之后的补丁型治理发布默认建议使用 `v0.1.6`，但执行前必须在确认句中写明具体版本；该确认包只覆盖 tag、GitHub Release workflow 和供应链记录，不覆盖服务器 updater apply、生产备份、migration、rollback 或 residual 台账关闭。
