...
| 平台化加固 | 基础版 | Core 已有队列退避/死信、Workspace 配额、固定窗口限流、容量状态和跨租户搜索可见性规则；本地候选新增 Operator-only 脱敏审计检索，以及 Operator/Workspace Owner 可见的只读容量快照，后者聚合成员、任务、24 小时导出/失败、附件字节和最老活动任务，固定 `OBSERVED_ONLY` 且不写死/执行配额；`UserNotification` 已提供默认关闭的幂等排名事件、鉴权 API、跨设备已读/未读/隐藏/恢复 UI 和导出预览联动。尚无通用持久队列/通知 worker、搜索索引 runtime、配额写入、MFA/Passkey、多租户指标/告警和灾备体验 | `tasks/backlog/0045-platform-hardening.md` |
