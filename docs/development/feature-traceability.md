...
| 错题 v2 学习证据闭环 | 隔离已实现 | `Mistake.questionText/correctAnswer/causeNote`、`MistakeAttempt`、`NoteMistakeLink`、模拟失分来源关联；`POST /api/mistakes/:id/attempts`、`PATCH /api/mistakes/:id/links`；详情独立作答与历史，统一复习事务双写，模拟失分预填创建；全新隔离库 36 migrations、M6 runtime 与 owner-isolation runtime 通过 | 尚未执行生产 migration、部署或 Release；OCR、AI 批改、自动判分、账户导出不在范围 |
