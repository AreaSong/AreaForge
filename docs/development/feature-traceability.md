...
| v1.2.0 发布候选 | 已完成 | 第一阶段已统一 package version 为 `1.2.0`，完成 additive migration 隔离验证、完整门禁、本地测试池与浏览器验收；PR #49 经 CI run `33505174259` 成功后 squash 合并到 `main` commit `c8b5acf241bcaada59c6b469fd562d4fe400d521`，main push CI run `33506280124` 也已成功 | tag/Release 仍需绑定最终 main commit 后单独确认；本阶段未执行生产 migration、生产更新、备份恢复或自动应用策略变更 |
