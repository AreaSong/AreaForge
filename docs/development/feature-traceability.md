...
| 邀请制多用户、Workspace 与 Membership | 隔离已实现 | 已形成账户状态/authRevision、设备 session、一次性 token、持久限流、SMTP、Membership/Invitation/Selection、Workspace CRUD/生命周期、邀请/成员/所有权生命周期、same-origin/CSRF 边界、actor/workspace fail-closed 和 owner-only 学习正文边界；隔离 PostgreSQL runtime、本地总门禁、Git 检查点及 commit `5816338` 的 branch push CI run `34021752169` 已通过。member-only 登录入口和成员只读考试页的管理查询旁路已修复，六类身份 desktop/mobile 可见性矩阵与跨 Workspace 404 通过，代表性邀请/角色/Operator 浏览器写入见 `output/playwright/v15-write-matrix/evidence.json`。默认多人开关关闭，完整失败变体、受保护 PR/合并、Release 和生产尚缺 | `tasks/active/0040-multi-user-rbac.md` |
| 预设角色、分享授权与 Coach 协作 | 隔离已实现 | v1.5 本地候选已形成 Admin/Coach/Viewer、统一 policy service、Owner-only 角色调整、USER/ROLE/WORKSPACE grant、NOTE/MISTAKE/ATTACHMENT 共享、CoachSuggestion/PlanInbox 确认链、脱敏 Operator 账户目录和无 Workspace 全局工具门禁；成员自有笔记候选、分享目标排除自己、App Shell/每日复盘的同 Workspace actor/owner 查询和六类身份 desktop/mobile 可见性矩阵已通过，证据见 `output/playwright/v15-role-matrix/evidence.json`；代表性角色、邀请、grant、Coach 建议确认和 Operator 账户状态/会话写入见 `output/playwright/v15-write-matrix/evidence.json`。commit `6566fab` 的 branch push CI run `34049447503` 已成功；当前本地 owner-required follow-up 已在全新 49-migration 隔离库通过非空约束、ownerless 拒绝、重复 deploy 和 v1.6-v1.8 兼容验证，并由检查点 `33fc4ab` / branch CI `34113486027` 通过服务层/API 失败矩阵。仍缺完整浏览器失败变体、受保护 PR/合并、Release 与生产证据 | `tasks/active/0044-rbac-privacy-collaboration.md` |
