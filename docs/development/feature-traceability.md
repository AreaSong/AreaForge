...
| 平台化加固 | 基础版 | Core 规则、脱敏审计、只读容量、直接数据库搜索和持久通知已有候选。内核/通知/EXPORT/DELETE 各有分域本地验收；DELETE 有 17 组隔离运行态和当前源码浏览器证据，OPS 有 38 组独立本地运行态，浏览器验收通过。独立 CLI 默认关闭，Web 不启动进程；排名重建、持久搜索、配额、MFA/Passkey、全域观测/灾备、共享与生产交付仍缺 | `tasks/backlog/0045-platform-hardening.md` |
| v2.0 综合门禁 | 未实现 | AUTH/RBAC/EXPORT/DELETE 已有分域本地证据，OPS 已有独立实现和 38 组运行态、真实 API/桌面/窄屏验收通过；排名重建、平台加固及全域回归、受保护合并、新签名 Release、生产与运营终态证据尚未齐备 | `tasks/backlog/0046-v2-platform-gate.md` |
