...
| 平台化加固 | 基础版 | Core 已有队列退避/死信、Workspace 配额、固定窗口限流、容量状态和跨租户搜索可见性规则；本地候选新增 Operator-only `GET /api/system/audit-events`，统一规范化 Workspace/actor/action/time/limit 查询并返回严格 allowlist 脱敏摘要；`UserNotification` 已提供默认关闭的幂等排名事件、鉴权 API、跨设备已读/未读/隐藏/恢复 UI 和导出预览联动。尚无通用持久队列/通知 worker、搜索索引 runtime、配额写入、MFA/Passkey、多租户指标/告警和灾备体验 | `tasks/backlog/0045-platform-hardening.md` |
