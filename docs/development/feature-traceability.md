...
| 平台化加固 | 基础版 | Core 已有队列退避/死信、Workspace 配额、固定窗口限流、容量状态和跨租户搜索可见性规则；本地候选新增 Operator-only 脱敏审计检索、Operator/Workspace Owner 只读容量快照，以及 ACTIVE Workspace 限定的鉴权搜索 API。搜索只返回结构信息和当前 actor 自有/有效 grant 资源的标题与 canonical href，不搜索私密正文并明确 `indexed=false`；容量固定 `OBSERVED_ONLY`，不写死/执行配额；`UserNotification` 已提供默认关闭的幂等排名事件、鉴权 API、独立通知路由、顶部栏入口、跨设备状态和导出预览。设置总览的运行版本、部署模式、自动更新策略和签名要求改为读取版本中心状态。持久 worker 执行内核已增加协议隔离、租约代次、退避/死信、暂停/取消/重放和事务提交，50 条 migration 与 15 组内核隔离 runtime（含两个真实进程 kill-point）通过；通知域已独立确认并通过 11 组隔离专项：完整协议/权限版本绑定、原子入队、幂等、撤销后重入、运行中开关、锁竞争/重放、24 个失效收件人业务场景、9 个所有权目标负向场景和通知副作用写入后但事务提交前的强杀恢复；独立 CLI 默认关闭；尚无排名重建、导出/删除处理器、通用持久搜索、配额写入、MFA/Passkey、多租户指标/告警和灾备体验 | `tasks/backlog/0045-platform-hardening.md` |
