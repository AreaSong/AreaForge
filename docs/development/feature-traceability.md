...
| 数据任务中心、回收站、物理删除与账户关闭 | 基础版 | 数据任务中心已支持 preview/排队/取消/重试、worker lease/CAS 和删除 preview；纯 Core 删除状态机默认不可执行并覆盖重新验证、冷静期、冻结、scope fingerprint、kill-point、补偿后重试，deletion ledger 覆盖哈希链与历史备份 replay plan。DELETE Web runtime 仍固定 preview-only，尚无统一回收站、物理删除、附件清理、账本持久化或真实重放；`AF-RISK-DATA-001` 保持 deferred-work | `tasks/backlog/0041-data-lifecycle.md` |
