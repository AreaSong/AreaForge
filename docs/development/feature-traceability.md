...
| 公共壳层、窗口、活动槽与 Dock | 已完成 | `GlobalTopBar`、`GlobalCommandPalette`（registry 支持 `$`/`/` 别名）、`PageToolbar`、`GlobalContextStatusBar`、`WindowSystemProvider`、`WindowDock`、`GlobalActivitySlot`、`GlobalConfirmationCenter`、`GlobalAiAssistant`、`GlobalQuickCreate`、`GlobalRecoveryHelp` 和 `GlobalSessionCloseout`；固定尺寸、单一前台、最小化/关闭策略、按父容器可用宽度动态展示后台窗口、本地持久化与跨标签页合并均有 `window-system-state.test.ts` 和导航契约覆盖；current-bound 本地浏览器复验覆盖主工作台、详情/复习运行页、390/768/900/1024/1440 宽度、底部状态详情、页面动作区、更多窗口菜单及 `↑/↓`、`Home/End`、`Escape` 键盘行为 | 后续只补充跨设备恢复等非本轮公共壳层范围的证据，不改变当前架构契约 |
