...
| 完整账户/Workspace 导出 | 基础版 | `packages/core` 已有 redaction/manifest/hash 和确定性内存 ZIP/真实字节 SHA-256；v1.6 本地候选增加 feature-gated 数据任务、脱敏预览/一次性 grant descriptor 和 worker claim/heartbeat/complete/expire 生命周期。账户 preview 已覆盖用户拥有或曾加入 Workspace 的上下文，Workspace preview 的 DataJob 已绑定当前 workspace；89 个 Prisma model 已全部分类，46 个接入 preview、37 个待最小化接入、5 个安全排除、1 个派生重建，并通过静态与隔离 runtime。仍未接真实临时存储、附件本体、保留/清理或下载流，不暴露 objectKey，不等于账户全量导出 | `tasks/backlog/0041-data-lifecycle.md` |
