...
| 邀请制多用户、Workspace 与 Membership | 隔离已实现 | 已形成账户状态/authRevision、设备 session、一次性 token、持久限流、SMTP、Membership/Invitation/Selection、Workspace CRUD/生命周期、邀请/成员/所有权生命周期、same-origin/CSRF 边界、actor/workspace fail-closed 和 owner-only 学习正文边界；隔离 PostgreSQL runtime、本地总门禁、Git 检查点及 commit `6568c87` 的 branch push CI run `34119581102` 已通过。member-only 登录入口和成员只读考试页的管理查询旁路已修复；六类身份 desktop/mobile 可见性、代表性写入和 17 项失败矩阵见 `output/playwright/v15-role-matrix/evidence.json`、`output/playwright/v15-write-matrix/evidence.json` 与 `output/playwright/v15-failure-matrix/evidence.json`。默认多人开关关闭，受保护 PR/合并、Release 和生产尚缺 | `tasks/active/0040-multi-user-rbac.md` |
| 预设角色、分享授权与 Coach 协作 | 隔离已实现 | v1.5 本地候选已形成 Admin/Coach/Viewer、统一 policy service、Owner-only 角色调整、USER/ROLE/WORKSPACE grant、NOTE/MISTAKE/ATTACHMENT 共享、CoachSuggestion/PlanInbox 确认链、脱敏 Operator 账户目录和无 Workspace 全局工具门禁；成员自有笔记候选、分享目标排除自己、App Shell/每日复盘的同 Workspace actor/owner 查询和六类身份 desktop/mobile 可见性矩阵已通过。代表性写入见 `output/playwright/v15-write-matrix/evidence.json`；17 项浏览器失败矩阵见 `output/playwright/v15-failure-matrix/evidence.json`，覆盖邀请/角色/grant/Coach/Operator、撤销/移除/暂停即时失效，意外 console/page error 和横向溢出为 0。owner-required follow-up 已在全新 49-migration 隔离库通过，并由检查点 `6568c87` / branch CI `34119581102` 验证 runner。受保护 PR/合并、Release 与生产证据仍缺 | `tasks/active/0044-rbac-privacy-collaboration.md` |
| 完整账户/Workspace 导出 | 基础版 | `packages/core` 已有 redaction/manifest/hash 和确定性内存 ZIP/真实字节 SHA-256；v1.6 本地候选增加 feature-gated 数据任务、脱敏预览/一次性 grant descriptor 和 worker claim/heartbeat/complete/expire 生命周期。账户 preview 已覆盖用户拥有或曾加入 Workspace 的上下文，Workspace preview 的 DataJob 已绑定当前 workspace；89 个 Prisma model 已全部分类，83 个接入 owner/actor-scoped preview、5 个安全排除、1 个派生重建，待最小化接入项已归零。静态门禁禁止选择 token/network hash、加密 API Key、nonce、幂等/请求指纹、objectKey、worker/lease 能力材料，隔离 runtime 已验证成员 session/挑战/参与/申诉与 Operator 自有受控请求。仍未接真实临时存储、附件本体、保留/清理或下载流，不暴露 objectKey，不等于账户全量导出 | `tasks/backlog/0041-data-lifecycle.md` |
...
| 个人成长指标、私有挑战与排名 | 基础版 | core 计分/策略、反作弊、opt-in、挑战/参与者 CRUD、可重建 projection、独立 projection 开关、持久申诉与成员/Owner UI、持久排名通知和删除 preview 已形成本地候选；通知只保存受控种类/Workspace 标签，不保存挑战名称或申诉正文。挑战、本人参与/申诉和通知均进入用户导出预览；隔离事务链证明成员关闭 opt-in 后参与转 `LEFT`、投影归零且账户删除预览为 `READY`，未转移挑战的 Owner 保持 `BLOCKED`。成员确认中心 scope 已修复；申诉与通知均有 390×844/1440×900 隔离浏览器证据。默认相关开关关闭，尚缺真实物理删除/完整归档、完整跨租户浏览器矩阵、共享/生产 migration 与生产证据 | `tasks/backlog/0043-ranking-platform-hardening.md` |
| 平台化加固 | 基础版 | Core 已有队列退避/死信、Workspace 配额、固定窗口限流、容量状态和跨租户搜索可见性规则；本地候选新增 Operator-only 脱敏审计检索、Operator/Workspace Owner 只读容量快照，以及 ACTIVE Workspace 限定的鉴权搜索 API。搜索只返回结构数据和当前 actor 自有/有效 grant 资源的标题与 canonical href，不搜索私密正文并明确 `indexed=false`；容量固定 `OBSERVED_ONLY`，不写死/执行配额；`UserNotification` 已提供默认关闭的幂等排名事件、鉴权 API、跨设备状态和导出预览。尚无通用持久队列/通知 worker、持久搜索索引、配额写入、MFA/Passkey、多租户指标/告警和灾备体验 | `tasks/backlog/0045-platform-hardening.md` |
