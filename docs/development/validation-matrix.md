...
| `docs/development/long-term-operability-control-plane.md`、`scripts/quality/enterprise-operability-preflight.ts`、长期运营控制面入口 | `pnpm enterprise:operability:preflight`，`pnpm maintenance:cadence:preflight`，`pnpm release:train:preflight`，`pnpm ops:readiness`，`pnpm residuals:validate`，`pnpm residuals:review-due`，`pnpm docs:readiness`，`pnpm skills:validate`，`git diff --check` |
| `docs/development/maintenance-cadence.md`、`scripts/quality/maintenance-cadence-preflight.ts`、维护节奏入口 | `pnpm maintenance:cadence:preflight`，`pnpm ops:readiness`，`pnpm residuals:validate`，`pnpm residuals:review-due`，`pnpm docs:readiness`，`pnpm skills:validate`，`git diff --check` |
