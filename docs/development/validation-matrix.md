...
| `docs/development/product-experience-review-record-template.md`、`scripts/quality/product-experience-review-validate.ts`、`scripts/quality/product-experience-review-validate.selftest.ts`、`scripts/ops/local-ux-smoke.ts`、`scripts/quality/local-ux-smoke.selftest.ts`、`docs/ux/**`、`.codex/skills-src/areaforge-product-experience/**`、`.codex/skills-src/areaforge-qa-smoke/**` | `pnpm experience:review:selftest`、`pnpm smoke:local-ux:selftest`、`pnpm docs:readiness`、`pnpm residuals:validate`、`pnpm skills:validate`、`git diff --check`；默认记录必须使用 `ux-source-v2` 扩展指纹并在 14 天新鲜度/300 秒未来偏差内。若形成真实体验记录，先运行 `pnpm experience:review:binding`，填截图路径后运行 `pnpm experience:review:hash <record> --print-record-hashes`，再运行 `pnpm experience:review:validate <record>`；若改 UI，补跑 `pnpm smoke:local-ux` 或浏览器/Playwright smoke，并明确记录是否绑定实际运行实例。local UX selftest 必须覆盖本地 URL、active-session 前置、密码文件 inode/权限/无明文 env 和结构化失败边界 |
...
| `docs/development/ops-006-business-state-concurrency-design.md`、`scripts/quality/ops006-concurrency-preflight.ts`、`scripts/quality/ops006-concurrency-preflight.selftest.ts`、OPS-006 确认前静态 preflight | `pnpm quality:operability:typecheck`、`pnpm ops:ops-006:preflight:selftest`、状态投影 `pnpm ops:ops-006:preflight`、确认边界断言 `pnpm ops:ops-006:preflight:strict`、`pnpm ops:data-integrity:selftest`、`pnpm db:validate`、`pnpm risk:preflight`、`pnpm docs:readiness`、`git diff --check`；`evidenceClass: migration_preimage_candidate` 只绑定 schema preimage、恰好一条规范 partial unique index 候选、24 小时内 `configured_read_only_query` doctor，以及 task/design/确认包 source hash 与 `OPS-006-PREFLIGHT-CONTRACT-V1`。candidate evidence complete 不等于 migration 获批/应用或 CAS 已实现；附件状态不参与该候选证据完整度。未经高风险实施确认，strict 必须非零退出。确认后另跑隔离 PostgreSQL 并发 start/pause/resume/end/task/CheckIn fixture 和 API 409 验证。 |
