...
| `scripts/dev/dev-test-*`、`scripts/quality/dev-test-pool.selftest.ts`、`infra/docker/web.dev-test.Dockerfile`、`package.json` 中 `dev:test:*`、本地测试池文档 | `pnpm dev:test:selftest`、`pnpm dev:test:typecheck`、`pnpm dev:test:latest -- --json`、`pnpm dev:test:doctor -- --json`、`pnpm dev:test:snapshot -- --dry-run --json`、`pnpm package-e:preflight`、`pnpm governance:preflight`、`pnpm docs:readiness`、`pnpm risk:preflight`、`pnpm check`、`git diff --check`；selftest 必须覆盖固定三槽、refresh 复用、snapshot FIFO、latest 不依赖槽位数字、FIFO 后 latest、失败候选不改变 latest、删除 latest 后回退、重复槽位/异名/端口拒绝、真实锁竞争和陈旧锁恢复，源码护栏必须保持 loopback bind、AI 外呼关闭、宿主机 standalone build 先于交换锁、测试镜像不重复安装依赖、失败回滚且无全局 prune/volume 删除。latest/doctor/dry-run 只证明当前本地池投影与选择计划，不证明真实镜像已构建、页面体验通过或生产状态变化 |
...
| Web 共享能力、`docs/architecture/web-shared-capability-inventory.json`、`apps/web/components/ui/**`、`apps/web/lib/api/**`、`apps/web/lib/client/**`、`apps/web/lib/contracts/**`、`scripts/quality/web-*-boundary*`、`scripts/quality/web-component-complexity*` | 运行五个专项 gate：`pnpm web:shared-boundary`、`pnpm web:api-parser-boundary`、`pnpm web:ui-primitives-boundary`、`pnpm web:client-boundary`、`pnpm web:component-complexity`；再运行对应五个 `*:selftest`、`pnpm web:governance:typecheck`、Web typecheck/lint/test、`pnpm check`、`pnpm docs:readiness` 和 `git diff --check`。Inventory 必须通过 JSON parse 和 `web:shared-boundary` schema/path/重叠校验；raw UI、直接 storage、显式 `401/409` 的 legacy budget 必须保持为空；组件 500 行为硬门禁，函数 50 行仅为 observation/warning。涉及 UI 行为时仍需刷新测试池并补当前源指纹的浏览器证据 |
...
| App Shell、共享布局原语或响应式页面迁移 | `pnpm check`；使用 `pnpm dev:test:refresh` 更新本任务对应的测试池实例并以 `pnpm dev:test:latest -- --json` 返回的 URL 验收；浏览器至少覆盖 `320x844`、`390x844`、`768x1024`、`820x1180`、`1024x768`、`1280x800`、`1440x1000` 和 `125%` 缩放，逐档检查页面根 `scrollWidth`、固定区域遮挡、长文本、工具栏收纳、键盘焦点和返回上下文。常规页与 `workspace-full` 分别只能有一个主纵向滚动所有者；有意横向滚动的区域必须有可见提示且不能藏住主操作。只迁移部分页面时不得据此声明全站响应式完成 |
...
| `docs/development/ops-006-production-evidence-template.md`、`scripts/quality/bound-json-evidence.ts`、`scripts/quality/ops006-production-evidence-validate*`、`scripts/ops/ops006-production-evidence-preflight.ts`、OPS-006 生产证据链 | `pnpm ops:ops-006:evidence:selftest`、`pnpm ops:ops-006:production:preflight:selftest`、`pnpm ops:ops-006:preflight:selftest`、带 current local doctor/runtime 的 strict local preflight、`pnpm release:supply-chain:selftest`、`pnpm sc:sc-002:preflight:selftest`、`pnpm release:evidence:selftest`、`pnpm ops:data-integrity:selftest`、`pnpm ops:long-term:gate:selftest`、`pnpm enterprise:operability:preflight`、`pnpm docs:readiness`、`pnpm residuals:validate`、`pnpm governance:preflight`、`pnpm secrets:scan`、`pnpm check`、`git diff --check`。先用 `pnpm ops:ops-006:confirmation-scopes <production-record-draft>` 生成 domain-separated rollout/probe scope；真实终态必须用 `pnpm ops:ops-006:evidence:validate <production-record> <release-record> <release-assets-dir> <release-evidence-record> <attachment-reconciliation.csv> <attachment-reconciliation-summary.json>`，并为基础 rollout 与 controlled probe 分别配置 confirmation ID 和 canonical scope SHA256 后运行 `pnpm ops:ops-006:production:preflight -- --require-human-review-ready`；它固定 24 小时 freshness，强制 strict assets/checksum/cosign、Release source-at-commit、生产 before/after doctor、canonical index、health/authenticated smoke、另行确认的 synthetic 409/单次副作用 probe、完整 Release evidence/reconciliation 和 rollback 绑定。`ready_for_ops006_human_review` 不自动关闭 residual。 |
...
| `ops/alerting/**`（告警推送 helper、systemd 单元、env 示例） | `pnpm shellcheck:updater`、`pnpm docs:links`、部署文档同步检查（`docs/deployment/alerting.md` 与 `docs/development/production-smoke-alerting-strategy.md` 阈值口径一致）；行为改动时用合成 env/status/backup fixture 本地跑一次 `check` 与 `notify --dry-run` 验证 severity/退出码/dedup 决策。helper 只读采集与推送，本地通过不代表服务器已安装 timer、真实接收人可达或 OPS-004 可关闭。 |
...
五个 gate 分别锁定不同边界，不能互相替代：

- `web:shared-boundary`：清单 schema、路径存在性/重叠、UI/contract/study 依赖方向，以及已移除 facade 不得恢复或导入。
- `web:api-parser-boundary`：浏览器组件和 client adapter 不直接 `fetch` 或解析 response body，统一使用 typed API adapter 和共享 parser。
- `web:ui-primitives-boundary`：非豁免 raw `input/select/textarea/button` 不得出现在业务 TSX；精确 legacy budget 保持为空。
- `web:client-boundary`：组件和 client 不直接访问浏览器 storage global，也不显式比较 HTTP `401/409`；两类精确 legacy budget 保持为空。
- `web:component-complexity`：非测试 TSX 不得超过 500 行，component legacy budget 保持为空；单函数超过 50 行只输出 observation/warning，不能被当作硬失败或完成阻断。

每个 gate 都必须有对应 `*:selftest`，治理脚本必须通过 `pnpm web:governance:typecheck`。`pnpm check` 聚合五个实际 gate，但不替代 selftest、当前 checkout 浏览器证据、Release 或 production apply。`apps/web/lib/contracts/study.ts`、`apps/web/lib/study/service.ts`、`apps/web/lib/study/types.ts` 已从架构契约中移除；任何重新创建或导入都必须失败。

客户端异步一致性改动还必须运行 `pnpm --filter @areaforge/web test` 和 typecheck。测试至少覆盖 latest-wins 乱序响应、显式 invalidate、同步互斥 acquire/release、旧批次释放隔离、revision 草稿 current/stale/legacy 三态、selection identity/fingerprint 分离及已知短哈希碰撞；UI 验收补充重复点击、pending 控件锁、context 切换、草稿恢复与 `409` 后输入保留。静态源码 selftest 拆分组件后必须跟随 canonical owner，不能继续只扫描拆分前主文件。
...
- 页面烟测：首页、`/roadmap/stages/trend`、`/roadmap/reviews` 刷新后连续性、低效天和低转化提示保持一致。
...
- 页面烟测：首页任务区、欠账预览和 `/roadmap/reviews` 对旧数据与新事件账本展示一致。
...
- 页面烟测：`/knowledge/syllabi` 条件、证据、复测、刷新后节点状态和历史 fallback 展示一致。
...
- 页面烟测：`/test/simulations` 和 `/roadmap/reviews` 中阶段计划、草稿边界和确认状态展示一致。
...
用户明确确认 Package A 后，才允许新增上传/下载 route、附件服务、`/knowledge/cards` 上传 UI 和真实 `UPLOAD_DIR` 写入。实现后至少运行：
...
- 页面烟测：`/knowledge/cards` 上传、刷新后附件列表、鉴权下载和响应头。
- 对账烟测：metadata hash/size 与文件 hash/size 一致；只读对账报告 `action=report_only`。

注意：Package A 完成后，`pnpm risk:preflight` 必须改为要求上传/下载 route、`attachments-service.ts`、`/knowledge/cards` 上传 UI、鉴权 `downloadApiPath` 和附件专项证据存在，同时继续阻止 public 暴露、内部 `uri` / `storedName` / 上传绝对路径泄露，以及 Package A 范围外的删除、跨对象附件、AI 解析和生产发布。
...
- 页面烟测：首页、`/roadmap/reviews`、`/roadmap/stages/trend`、`/knowledge/syllabi`、`/test/simulations` 展示确认边界和应用结果。
...
| Batch D4 长期风险和主题闭环 | 已完成：`GET /api/analytics/long-term-risks` 鉴权 GET-only；`long-term-risk-service` 调用 `summarizeLongTermRisks` 并保留 `evidenceFreshness`、`nextAction`、`canAutoApply=false`、`requiresUserConfirmation=true`；`/roadmap/reviews`、`/roadmap/stages/trend`、`/knowledge/syllabi`、`/knowledge/cards`、`/test/simulations` 和首页状态主题共用同一长期风险 DTO；service/route smoke 证明业务表不变 |
...
- 发布后烟测：`GET /api/health`、登录、首页、任务、计时、复盘、`/knowledge/syllabi`、`/knowledge/cards`、`/roadmap/stages/trend`、`/roadmap/reviews`；附件和真实 AI 若启用，只用小测试文件和最小测试数据。
...
- 覆盖：ZIP/MD policy、FILE/LINK、confirm 原子/幂等/409、history owner、一次性导出；页面入口为 `/knowledge/imports` 与 `/knowledge/resources`；不跑生产 migration
...
- Product Experience 记录必须直接绑定 `v11-browser-journey-evidence-v1` 路径/hash；独立无障碍记录必须直接绑定 `v11-accessibility-evidence-v1` 路径/hash。`pnpm release:v11:admission` 会以同一 `sourceGitCommit / releaseTag 派生的精确 1.1.patch / productExperienceSourceHash` 再次调用正式 validator，并在最终阶段重读所有嵌套证据。
...
通知处理器还须通过 `pnpm --filter @areaforge/db test` 中的 payload/allowlist selftest 与隔离 runtime 的合成幂等写入。
...
心跳/中止、旧协议隔离和两个真实进程 kill-point；不得把无数据库的普通 check 或 CI 当作该 runtime 的替代证据。
