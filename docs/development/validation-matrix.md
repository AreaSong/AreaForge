...
| `docs/development/long-term-operability-control-plane.md`、`docs/development/support-bundle-preview.md`、`scripts/quality/enterprise-operability-preflight.ts`、`scripts/ops/operability-status.ts`、`scripts/ops/operational-handoff.ts`、`scripts/ops/support-bundle-preview.ts`、长期运营控制面入口 | `pnpm enterprise:operability:preflight`，`pnpm maintenance:cadence:preflight`，`pnpm release:train:preflight`，`pnpm ops:readiness`，`pnpm ops:status`，`pnpm ops:status:selftest`，`pnpm ops:handoff`，`pnpm ops:handoff:selftest`，`pnpm ops:support:bundle-preview:selftest`，`pnpm residuals:validate`，`pnpm residuals:review-due`，`pnpm docs:readiness`，`pnpm skills:validate`，`git diff --check` |
