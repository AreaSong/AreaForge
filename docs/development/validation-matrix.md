...
| `docs/development/maintenance-window-record-template.md`、`scripts/quality/maintenance-window-record-validate.ts`、`scripts/quality/maintenance-window-record-validate.selftest.ts` | `pnpm maintenance:window:selftest`，`pnpm maintenance:cadence:preflight`，`pnpm enterprise:operability:preflight`，`pnpm docs:readiness`，`git diff --check`；若形成真实维护窗口记录，运行 `pnpm maintenance:window:validate <record>` |
