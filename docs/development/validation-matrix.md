...
| `scripts/dev/dev-test-*`、`scripts/quality/dev-test-pool.selftest.ts`、`infra/docker/web.dev-test.Dockerfile`、`package.json` 中 `dev:test:*`、本地测试池文档 | `pnpm dev:test:selftest`、`pnpm dev:test:typecheck`、`pnpm dev:test:doctor -- --json`、`pnpm dev:test:snapshot -- --dry-run --json`、`pnpm package-e:preflight`、`pnpm governance:preflight`、`pnpm docs:readiness`、`pnpm risk:preflight`、`pnpm check`、`git diff --check`；selftest 必须覆盖固定三槽、refresh 复用、snapshot FIFO、重复槽位/异名/端口拒绝、真实锁竞争和陈旧锁恢复，源码护栏必须保持 loopback bind、AI 外呼关闭、宿主机 standalone build 先于交换锁、测试镜像不重复安装依赖、失败回滚且无全局 prune/volume 删除。doctor/dry-run 只证明当前本地池结构与选择计划，不证明真实镜像已构建、页面体验通过或生产状态变化 |
