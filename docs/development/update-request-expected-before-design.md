...
6. 完成目标下载/签名预检和只读准备后，在 backup/updater/config/Docker 副作用的最后边界重新校验 TTL、
   ...
   快照继续执行。
7. 任一次不匹配都写入不可变 history 结果并停止；Release tag/target 不一致必须输出结构化
   `TARGET_IDENTITY_CHANGED` 零副作用 marker。updater reconciliation marker 只有在唯一 first/second pass、唯一
   execution marker 和 `executionAttempted=true` 同时存在时才可证明具体不确定副作用；缺失、重复或矛盾证据
   统一进入 `UPDATER_GUARD_EVIDENCE_INVALID`，不得只凭 marker 文本或退出码推断已经执行。
8. mutation 的不可变 decision、redacted `status.json` 终态发布和 claim 清理必须按该顺序在释放 shared lock
   前完成；status file 或目录 fsync 失败时保留 claim，重启从 immutable history 重新发布 status 后再清理，
   避免 direct updater 插入另一轮状态变化，也避免已删 claim 后只留下陈旧 status。
...
- `AUTO_APPLY_WRITE_FAILED`
- `AUTO_APPLY_PERSISTENCE_UNCERTAIN`
- `AUTO_APPLY_STATE_UNCERTAIN`
...
- 同一 processing claim 出现多个 request JSON 时，必须生成持久的
  `PROCESSING_CLAIM_AMBIGUOUS` reconciliation blocker，保留全部文件且不得选取第一份执行或清理其余证据。
...
- decision 已持久化但 status file/directory fsync 失败时，claim 必须保留；重启只能复用既有 decision
  重发 status，成功后再清 claim，不得重放 updater、rollback 或 policy mutation。
- 同一 processing claim 出现多个 request JSON 时必须稳定复用同一
  `PROCESSING_CLAIM_AMBIGUOUS` immutable decision，不得重复追加 history 或删除任一未解释请求。
- verified Release ID 在 Web、agent 和 updater 三层都必须是正的 JavaScript-safe integer；Web snapshot
  parser 对越界、负数、零和小数统一返回 invalid，不得在 canonical hash 阶段抛出未分类异常。
- target version 等于或低于 current version 时，updater request guard 必须输出结构化
  `TARGET_VERSION_NOT_NEWER`、`executionAttempted=false`，并在 backup/execution marker 前结束。
...
- production env、rollback source record、auto-apply config 和 decision history 的 file/directory fsync 失败均停在
  可协调边界。策略临时文件在 rename 前失败且确认值未变化时记录 `AUTO_APPLY_WRITE_FAILED` 零副作用终态；
  rename 后目录 fsync 失败且已观察到目标值时记录 `AUTO_APPLY_PERSISTENCE_UNCERTAIN` 并保留 claim，无法确认值时
  使用 `AUTO_APPLY_STATE_UNCERTAIN`。
