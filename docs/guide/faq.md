...
**任务名额或导出次数已满，会影响学习吗？**
不会限制学习任务或搜索直查。名额满时可取消不再需要的任务，或等待完成/到期；暂停与可重放失败仍占名额。
导出次数按滚动 24 小时计算，取消不退次数；相同请求的重试不重复计量。坏配额配置只阻止新受控请求，已有任务仍按原规则操作。
配额按本人工作区或账户分区分别计算，参见[后台任务准入配额](../modules/data-job-quotas.md)。

**为什么搜索显示“安全直查”，或看不到之前的结果？**
索引关闭、未建立、失效或超过安全容量时，系统直接查询当前获授权的源记录；仍可在“设置 → 数据与安全”申请重建。
撤权、归档或冻结后旧标题会隐藏，重建不会恢复访问权。状态读取失败可刷新，申请回执不明则重试同一请求；参见[工作区持久搜索](../modules/workspace-search.md)。
