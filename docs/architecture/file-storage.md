...
下一产品版本在既有 Attachment staging 协议之上增加 `StudyResource`：FILE 只引用 READY 附件，LINK 只保存规范化 HTTPS URL；资料与笔记附件身份分离；重复文件走复用/副本/跳过三选一。不物理删除资料或历史 orphan。权威契约见 `workflow/versions/v1.1-learning-action-center.md`。
