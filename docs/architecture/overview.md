...
- 长期运营入口已补齐：release workflow 先验证再发布，stable 签名 fail closed；`v0.1.7` 已生成并校验 SBOM/provenance、checksum、cosign signature 和 GHCR digest 证据；`operational-readiness.md` 记录只读运营证据模型；`residual-risk-ledger.md` 记录生产 extra smoke、自动策略、签名 Release 复核等残余风险 ID。
