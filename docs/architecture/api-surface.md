...
Package D Batch D4 后，`GET /api/analytics/long-term-risks` 是长期风险统一 DTO 的只读鉴权入口，返回风险来源、时间窗口、科目/考纲节点、证据新鲜度、下一步动作、`canAutoApply=false` 和 `requiresUserConfirmation=true`。该 API 不新增长期风险状态、不修改任务或阶段计划、不触发长期 AI。
