...
- v1.6 纯 Core 候选只在内存中构造已脱敏的确定性 ZIP，并用真实字节 SHA-256 绑定 manifest；写出必须经显式注入 sink，不能读取 `UPLOAD_DIR`、数据库、备份或网络。删除状态机默认不可执行，只演练重新验证、冷静期、冻结、范围 fingerprint、kill-point 和补偿重试。物理删除资料或导入历史、备份副本同步删除、用户迁移与完整账户导出服务仍不在当前可执行范围。
