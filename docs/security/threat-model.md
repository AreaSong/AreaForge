...
- 本人分区限额与活跃任务总量分别控制；总量聚合本人跨工作区、工作区跨请求者和实例三域任务，不是支付计费或真实进程并发保证。配置不能由请求体覆盖，也不公开其他请求者的使用量。
...
成员席位按 canonical Owner 预留与原始 ACTIVE membership 计量；停用/冻结不能提前释放，不能使用面向可见性的 count。
邀请接受的权限、一次性凭证和席位检查必须与账户/成员/邀请变化同事务提交，拒绝不能留下半个新账户；并发旧快照必须中止。
限额不驱逐存量、不拦个人工作区建立和退出/移除；详见[成员席位与活跃任务总量](../modules/capacity-quotas.md)。
