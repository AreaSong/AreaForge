...
import { guardDerivedQueueTransaction, isDerivedQueueKind } from "./data-job-derived-guard";
...
export async function heartbeatQueuedDataJob(client: DataQueueClient, input: { lease: DataJobLease; leaseMs: number; progress?: number }): Promise<"RUNNING" | "PAUSED" | "CANCELLED"> {
  ...
  return client.$transaction(async (tx) => {
    await guardDerivedQueueTransaction(tx, [input.lease.kind], input.lease.jobId);
...
export async function commitQueuedDataJob(client: DataQueueClient, input: {
  ...
  return client.$transaction(async (tx) => {
    ...
    return "SUCCEEDED";
  }, { timeout: 15_000, ...(isDerivedQueueKind(input.lease.kind) ? { isolationLevel: "Serializable" as const } : {}) });
}
