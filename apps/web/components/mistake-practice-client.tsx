...
} from "@/lib/study/mistake-practice";
import type { MistakeCauseDto, MistakeDto, MistakeAttemptDto, SubjectDto, SyllabusOptionNodeDto } from "@/lib/study/types";
...
export function MistakePracticeClient(props: {
  ...
  async function saveCurrent() {
    ...
    const scope = `mistake-practice:${props.userId}:${sessionId}:${current.id}:${index}`;
    setPending(true);
    setError(null);
    try {
      const response = await fetch(`/api/mistakes/${current.id}/attempts`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idempotencyKey: getOrCreateIdempotencyKey(scope, "mistake-practice-attempt", payload), ...payload }),
      });
      const body = await response.json().catch(() => null) as { attempt?: MistakeAttemptDto; error?: string } | null;
      if (!response.ok || !body?.attempt) throw new Error(body?.error ?? "保存练习结果失败，请重试。");
      completeIdempotentCommand(scope);
      const nextResults = [...results, { mistake: current, attempt: body.attempt }];
      ...
      setError(caught instanceof Error ? caught.message : "保存练习结果失败，请重试。");
    } finally {
      setPending(false);
    }
  }
...
function PracticeQuestion(props: {
  ...
  return <div className="mx-auto w-full max-w-4xl space-y-6">
    <PageHeader eyebrow={`错题练习 · ${props.index + 1} / ${props.total}`} title={current.title} description={`${current.subjectName} · ${current.syllabusNodeTitle ?? "未关联考纲"}`} back={<button type="button" onClick={props.onBack} className="inline-flex items-center gap-1 text-sm text-teal-300 hover:underline"><ArrowLeft size={15} aria-hidden />退出本轮</button>} />
    ...
    <section className="space-y-4 rounded-lg border border-white/10 bg-[#101419] p-5 sm:p-7">
      ...
      {!props.revealed ? <>
        <fieldset className="flex flex-wrap gap-2 border-t border-white/10 pt-4"><legend className="mb-2 text-sm text-zinc-400">作答方式</legend>{([['TEXT', '文字作答'], ['PAPER_OR_ORAL', '纸上 / 口头']] as const).map(([value, label]) => <label key={value} className={`flex h-10 cursor-pointer items-center gap-2 rounded-md border px-3 text-sm ${props.answerMode === value ? "border-teal-300/50 bg-teal-300/10 text-teal-100" : "border-white/10 text-zinc-300"}`}><input type="radio" name="practice-answer-mode" value={value} checked={props.answerMode === value} onChange={() => props.onAnswerModeChange(value)} />{label}</label>)}</fieldset>
        {props.answerMode === "TEXT" ? <textarea aria-label="练习作答" className="min-h-36 w-full rounded-md border border-white/10 bg-[#151a20] px-3 py-2 text-sm leading-6" value={props.answerText} onChange={(event) => props.onAnswerTextChange(event.target.value)} placeholder="写下答案或关键步骤" /> : <label className="flex items-center gap-3 rounded-md border border-white/10 p-4 text-sm text-zinc-200"><input type="checkbox" checked={props.paperCompleted} onChange={(event) => props.onPaperCompletedChange(event.target.checked)} />我已在纸上或口头完成独立作答</label>}
      ...
      </> : <div className="space-y-4 border-t border-white/10 pt-4"><div className="grid gap-4 sm:grid-cols-2"><div><p className="text-xs text-zinc-500">标准答案</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-200">{current.correctAnswer || "未记录标准答案"}</p></div><div><p className="text-xs text-zinc-500">正确思路</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-200">{current.correctIdea}</p></div></div><fieldset className="flex flex-wrap gap-2"><legend className="mb-2 w-full text-sm text-zinc-400">本次结果</legend>{([['PASSED', '通过'], ['PARTIAL', '部分掌握'], ['FAILED', '未通过']] as const).map(([value, label]) => <label key={value} className={`flex h-10 cursor-pointer items-center gap-2 rounded-md border px-3 text-sm ${props.result === value ? "border-teal-300/50 bg-teal-300/10 text-teal-100" : "border-white/10 text-zinc-300"}`}><input type="radio" name="practice-result" value={value} checked={props.result === value} onChange={() => props.onResultChange(value)} />{label}</label>)}</fieldset><textarea aria-label="练习复盘备注" className="min-h-24 w-full rounded-md border border-white/10 bg-[#151a20] px-3 py-2 text-sm leading-6" value={props.note} onChange={(event) => props.onNoteChange(event.target.value)} placeholder="记录卡点、遗漏或下次注意事项" /><div className="flex flex-wrap gap-2"><Button type="button" variant="primary" disabled={props.pending} onClick={props.onSave}><Save size={16} aria-hidden />{props.pending ? "保存中" : props.index + 1 === props.total ? "保存并完成练习" : "保存并进入下一题"}</Button><button type="button" className="inline-flex h-10 items-center gap-1 px-2 text-sm text-teal-300" onClick={props.onBackToAnswer}><RotateCcw size={15} aria-hidden />重新核对作答</button></div></div>}
...
function Stat({ label, value }: { label: string; value: string }) { return <div className="border-l border-white/10 pl-3"><p className="text-xs text-zinc-500">{label}</p><p className="mt-1 text-lg font-semibold text-white">{value}</p></div>; }
