...
export function KnowledgePointDetail({ knowledgePoint }: { knowledgePoint: KnowledgePointDetailDto }) {
  ...
  const [masteryState, setMasteryState] = useState<KnowledgeMasteryStateDto>(knowledgePoint.masteryState);
  ...
  function save() {
    ...
    startTransition(async () => {
      const response = await fetch(`/api/knowledge-points/${knowledgePoint.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ expectedRevision: revision, title, boundary: boundary.trim() || null, masteryState, nextRetestAt: nextRetestAt ? new Date(`${nextRetestAt}T00:00:00.000Z`).toISOString() : null }) });
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-4 border-b border-white/10 pb-6 sm:grid-cols-3">{Object.entries(knowledgePoint.counts).map(([key, value]) => <div key={key} className="border-l border-white/10 pl-3"><p className="text-xs text-zinc-500">{countLabel(key)}</p><p className="mt-1 text-2xl font-semibold text-white">{value}</p></div>)}</section>
      ...
      <section className="space-y-4 border-b border-white/10 pb-6">
        ...
        <div className="grid gap-4 lg:grid-cols-2"><label className="grid gap-1 text-xs text-zinc-400">名称<input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={180} className="h-10 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100 outline-none focus:border-teal-400/50" /></label><label className="grid gap-1 text-xs text-zinc-400">掌握状态<select value={masteryState} onChange={(event) => setMasteryState(event.target.value as KnowledgeMasteryStateDto)} className="h-10 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100">{Object.entries(masteryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="grid gap-1 text-xs text-zinc-400 lg:col-span-2">知识边界<textarea value={boundary} onChange={(event) => setBoundary(event.target.value)} rows={4} maxLength={3000} className="rounded-md border border-white/10 bg-[#0b0e12] px-3 py-2 text-sm text-zinc-100 outline-none focus:border-teal-400/50" /></label><label className="grid gap-1 text-xs text-zinc-400">下一次复测日期<input type="date" value={nextRetestAt} onChange={(event) => setNextRetestAt(event.target.value)} className="h-10 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100" /></label></div>
      ...
      <RelationSection title="阶段目标" empty="还没有进入阶段目标。">{knowledgePoint.stageTargets.map((target) => <li key={target.id} className="flex items-center justify-between gap-3 py-3"><span className="text-sm text-zinc-200">{target.stage?.name ?? "未命名阶段"}</span><span className="text-xs text-zinc-500">目标：{masteryLabels[target.targetState]} · 重要度 {target.importance}</span></li>)}</RelationSection>
