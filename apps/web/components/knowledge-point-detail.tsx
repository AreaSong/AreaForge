  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4 border-b border-white/10 pb-6">
        ...
        <div className="af-content-grid-two grid min-w-0 gap-4"><label className="grid min-w-0 gap-1 text-xs text-zinc-400">名称<input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={180} className="h-10 min-w-0 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100 outline-none focus:border-teal-400/50" /></label><label className="grid min-w-0 gap-1 text-xs text-zinc-400">掌握状态<select value={masteryStatus} onChange={(event) => setMasteryStatus(event.target.value as MasteryStatus)} className="h-10 min-w-0 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100">{MASTERY_STATUS_OPTIONS.map((status) => <option key={status} value={status}>{masteryStatusLabel(status)}</option>)}</select></label><label className="af-content-span-all grid min-w-0 gap-1 text-xs text-zinc-400">知识边界<textarea value={boundary} onChange={(event) => setBoundary(event.target.value)} rows={4} maxLength={3000} className="min-w-0 rounded-md border border-white/10 bg-[#0b0e12] px-3 py-2 text-sm text-zinc-100 outline-none focus:border-teal-400/50" /></label><label className="grid min-w-0 gap-1 text-xs text-zinc-400">下一次复测日期<input type="date" value={nextRetestAt} onChange={(event) => setNextRetestAt(event.target.value)} className="h-10 min-w-0 rounded-md border border-white/10 bg-[#0b0e12] px-3 text-sm text-zinc-100" /></label></div>
