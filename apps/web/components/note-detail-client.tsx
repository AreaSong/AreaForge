...
export function NoteDetailClient(props: {
  ...
  async function changeArchiveState(intent: "archive" | "restore") {
    ...
    if (intent === "archive" && !window.confirm("归档后卡片将变为只读，活动复习排期会同时暂停。确认归档？")) return;
  ...
  return (
    <article className="space-y-6" aria-busy={Boolean(pending)}>
      <BackToListLink className="text-sm text-teal-300 hover:underline" fallbackHref="/knowledge/notes">
        返回卡片列表
      </BackToListLink>

      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-5">
        <div className="min-w-0">
          <p className="text-xs text-zinc-500">{kindLabel(note.kind)} · {note.subjectName} · r{note.revision}</p>
          <DetailHeading className="mt-1 break-words text-2xl font-semibold text-white">{note.title}</DetailHeading>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {props.readOnly ? null : archived ? (
            ...
            <button type="button" disabled={Boolean(pending)} onClick={() => void save()} className="inline-flex h-10 items-center gap-2 rounded-md bg-teal-500 px-3 text-sm font-medium text-black disabled:opacity-50">
              <Save size={16} aria-hidden />{pending === "save" ? "保存中" : "保存卡片"}
            </button>
          ) : reviewDue && note.reviewSchedule ? (
            <Link href={`/quick-review/${note.reviewSchedule.id}?returnTo=${encodeURIComponent(`/knowledge/notes/${note.id}`)}`} className="inline-flex h-10 items-center gap-2 rounded-md bg-teal-500 px-3 text-sm font-medium text-black">
          ...
          {!props.readOnly && !archived ? (
            <button type="button" title="归档卡片" aria-label="归档卡片" disabled={Boolean(pending)} onClick={() => void changeArchiveState("archive")} className="grid size-10 place-items-center rounded-md border border-white/10 text-zinc-300 disabled:opacity-50">
              <Archive size={16} aria-hidden />
            </button>
          ) : null}
          {!props.readOnly && editing ? (
            <button type="button" title="放弃本地编辑" aria-label="放弃本地编辑" disabled={Boolean(pending)} onClick={discardDraft} className="grid size-10 place-items-center rounded-md border border-white/10 text-zinc-300 disabled:opacity-50">
...
function ReviewHistory(props: { note: NoteDto; archived: boolean; readOnly: boolean; pending: boolean; reviewDate: string; scheduleCanBeCreated: boolean; onReviewDateChange: (value: string) => void; onSchedule: () => void }) {
  const schedule = props.note.reviewSchedule;
  return (
    <section className="border-t border-white/10 pt-5" aria-labelledby="note-review-heading">
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 id="note-review-heading" className="text-lg font-medium text-white">复习排期与历史</h2>{schedule && !props.readOnly ? <Link className="text-sm text-teal-300 hover:underline" href={`/knowledge/reviews/${schedule.id}?returnTo=${encodeURIComponent(`/knowledge/notes/${props.note.id}`)}`}>打开排期详情</Link> : null}</div>
