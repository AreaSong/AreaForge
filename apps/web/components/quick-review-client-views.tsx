  ...
  return (
    <section className="mx-auto flex min-h-full max-w-2xl flex-col justify-center gap-5 px-4 py-8">
      ...
      <dl className="grid grid-cols-2 gap-3 rounded-md border border-white/10 bg-[#101419] p-4 text-sm">
        <div><dt className="text-zinc-500">本次结果</dt><dd className="mt-1 text-white">{reviewResultLabel(props.done.event.result)}</dd></div>
        <div><dt className="text-zinc-500">有效时长</dt><dd className="mt-1 text-white">{formatShortDuration(props.done.event.durationSeconds)}</dd></div>
        <div><dt className="text-zinc-500">下次复习</dt><dd className="mt-1 text-white">{formatDate(props.done.event.nextDueDate)}</dd></div>
        <div><dt className="text-zinc-500">掌握变化</dt><dd className="mt-1 text-white">{masteryChangeLabel(props.schedule.consecutivePassCount, props.done.schedule.consecutivePassCount)}</dd></div>
      </dl>
...
export function QuickReviewForm(props: {
    ...
    return (
      <div className="space-y-4 rounded-md border border-white/10 bg-[#101419] p-4">
        ...
        <fieldset disabled={props.activityBlocked} className="space-y-2 disabled:opacity-60">
          ...
          <label className="flex items-center gap-2 text-sm"><Radio name="quick-review-answer-mode" checked={props.draft.answerMode === "TEXT"} onChange={() => props.onUpdate({ answerMode: "TEXT" })} />文字作答</label>
          <label className="flex items-center gap-2 text-sm"><Radio name="quick-review-answer-mode" checked={props.draft.answerMode === "PAPER_OR_ORAL"} onChange={() => props.onUpdate({ answerMode: "PAPER_OR_ORAL" })} />纸上或口头作答</label>
        </fieldset>
        {props.draft.answerMode === "TEXT" ? (
          <label className="block text-sm">你的答案<Textarea disabled={props.activityBlocked} className="mt-1 min-h-24 px-2 disabled:opacity-60" value={props.draft.answerText} onChange={(event) => props.onUpdate({ answerText: event.target.value })} /></label>
        ) : (
          <label className="flex items-center gap-2 text-sm"><Checkbox disabled={props.activityBlocked} checked={props.draft.paperOrOralCompleted} onChange={(event) => props.onUpdate({ paperOrOralCompleted: event.target.checked })} />我已完成纸上或口头作答</label>
        )}
  ...
  return (
    <div className="space-y-3 rounded-md border border-white/10 bg-[#101419] p-4">
      {props.targetType === "MISTAKE" ? <p className="text-xs text-zinc-500">已完成{props.draft.answerMode === "TEXT" ? "文字" : "纸上或口头"}作答</p> : null}
      <label className="block text-sm">结果<Select disabled={props.activityBlocked} className="mt-1 px-2 disabled:opacity-60" value={props.draft.result} onChange={(event) => props.onUpdate({ result: event.target.value as QuickReviewResult })}><option value="PASSED">通过</option><option value="PARTIAL">部分</option><option value="FAILED">失败</option></Select></label>
      <label className="block text-sm">下次日期（可选）<Input type="date" disabled={props.activityBlocked} className="mt-1 px-2 disabled:opacity-60" value={props.draft.nextDueDate} onChange={(event) => props.onUpdate({ nextDueDate: event.target.value })} /></label>
      <label className="block text-sm">备注<Textarea disabled={props.activityBlocked} className="mt-1 min-h-20 px-2 disabled:opacity-60" value={props.draft.note} onChange={(event) => props.onUpdate({ note: event.target.value })} /></label>
      <div className="flex flex-wrap gap-2">
        ...
        <Button type="button" disabled={props.activityBlocked} className="h-11 px-4" onClick={props.onDiscard}>丢弃草稿</Button>
      </div>
