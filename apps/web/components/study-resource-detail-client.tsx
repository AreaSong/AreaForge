    ...
    <article className="space-y-6">
      ...
      <KnowledgeNextAction
        ...
        action={!archived && resource.taskIds[0] ? (
          <Link href={withReturnTo(`/roadmap/arrangements/tasks/${resource.taskIds[0]}`, objectHref)} className="inline-flex h-10 items-center gap-2 rounded-md border border-teal-300/30 px-3 text-sm text-teal-100 hover:bg-teal-300/10"><CalendarCheck size={16} aria-hidden />开始关联任务<ArrowRight size={16} aria-hidden /></Link>
        ) : !archived ? (
          <Link href={`/roadmap/arrangements?subjectId=${encodeURIComponent(resource.subjectId ?? "")}&resourceId=${encodeURIComponent(resource.id)}`} className="inline-flex h-10 items-center gap-2 rounded-md bg-teal-400 px-3 text-sm font-medium text-[#071011] hover:bg-teal-300"><CalendarCheck size={16} aria-hidden />创建学习任务<ArrowRight size={16} aria-hidden /></Link>
        ) : null}
      />
    ...
    <>
      ...
      <section className="space-y-4 border-t border-white/10 pt-5" aria-labelledby="resource-associations-heading">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <AssociationLinks label="任务" ids={props.resource.taskIds} options={props.options.tasks} hrefFor={(id) => withReturnTo(`/roadmap/arrangements/tasks/${id}`, props.objectHref)} />
