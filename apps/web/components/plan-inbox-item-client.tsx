...
export function PlanInboxItemClient({ userId, item: initialItem, options, returnTo: initialReturnTo }: { userId: string; item: PlanInboxItemDto; options: PlanInboxFormOptions; returnTo?: string }) {
  ...
  const returnTo = initialReturnTo ?? "/plan/inbox";
  ...
  function applyServerVersion(latest: PlanInboxItemDto): void {
    ...
    if (latest.id !== initialItem.id) router.replace(withReturnTo(`/plan/inbox/${latest.id}`, returnTo));
  }
  ...
  function adoptLatestRevisionForManualMerge(latest: PlanInboxItemDto): void {
    ...
    if (latest.id !== initialItem.id) {
      ...
      setDraftReady(false);
      removePrivateBusinessDraft(formDraftKey);
      router.replace(withReturnTo(`/plan/inbox/${latest.id}`, returnTo));
    }
    setItem(latest);
    setBaseRevision(latest.revision);
  ...
  return (
    <section className="space-y-5">
      ...
      {item.supersededByItemId ? <p className="rounded-md border border-amber-300/20 bg-amber-400/5 p-3 text-sm text-amber-200">已被新版本取代。<Link href={withReturnTo(`/plan/inbox/${item.supersededByItemId}`, detailHref(item.id, returnTo))} className="ml-2 text-teal-300 hover:underline">查看最新</Link></p> : null}
      {pendingConvert ? (
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {item.convertedTaskId ? <Link href={withReturnTo(`/plan/tasks/${item.convertedTaskId}`, returnTo)} className="h-11 rounded-md bg-teal-500/90 px-4 text-sm font-medium leading-[44px] text-black">打开任务</Link> : null}
      </div>
      {error ? <p role="alert" className="text-sm text-red-300">{error}</p> : null}
      {item.requiredMilestoneKey && !item.planMilestoneId ? <div className="flex flex-wrap items-center gap-2 text-sm text-amber-200"><span>此草稿引用 canonical 里程碑 {item.requiredMilestoneKey}。</span><button type="button" className="text-teal-300 hover:underline disabled:opacity-50" disabled={busy || Boolean(pendingConvert) || !options.stagePlans.length} onClick={() => void createRequiredMilestone()}>创建并选中</button>{!options.stagePlans.length ? <Link className="text-teal-300 hover:underline" href={withReturnTo("/plan/stages", detailHref(item.id, returnTo))}>先创建阶段计划</Link> : null}</div> : null}
...
function detailHref(itemId: string, returnTo: string): string {
  return withReturnTo(`/plan/inbox/${itemId}`, returnTo);
}
