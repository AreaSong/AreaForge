  ...
  return (
    <article className="space-y-6">
      ...
      <section className="space-y-3 border-t border-white/10 pt-5" aria-labelledby="mistake-answer-heading">
        ...
        {!revealed ? <>
          <fieldset className="flex flex-wrap gap-2"><legend className="mb-2 text-sm text-zinc-400">作答方式</legend>{([['TEXT', '文字作答'], ['PAPER_OR_ORAL', '纸上 / 口头']] as const).map(([value, label]) => <label key={value} className={`flex h-10 cursor-pointer items-center gap-2 rounded-md border px-3 text-sm ${answerMode === value ? 'border-teal-300/50 bg-teal-300/10 text-teal-100' : 'border-white/10 text-zinc-300'}`}><Radio name="answer-mode" value={value} checked={answerMode === value} onChange={() => setAnswerMode(value)} />{label}</label>)}</fieldset>
          {answerMode === "TEXT" ? <Textarea aria-label="本次作答" className="min-h-32 px-3 leading-6" value={answerText} onChange={(event) => setAnswerText(event.target.value)} placeholder="写下本次答案或关键步骤" /> : <label className="flex items-center gap-3 rounded-md border border-white/10 p-4 text-sm text-zinc-200"><Checkbox checked={paperOrOralCompleted} onChange={(event) => setPaperOrOralCompleted(event.target.checked)} />我已在纸上或口头完成独立作答</label>}
          ...
          <div className="af-content-grid-two grid gap-3"><div><p className="text-xs text-zinc-500">标准答案</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-200">{mistake.correctAnswer || "未记录标准答案"}</p></div><div><p className="text-xs text-zinc-500">正确思路</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-200">{mistake.correctIdea || "待补全"}</p></div></div>
          {!attemptSaved && !props.readOnly && !archived ? <div className="space-y-3 border-t border-white/10 pt-3"><fieldset><legend className="mb-2 text-sm text-zinc-400">本次结果</legend><div className="flex flex-wrap gap-2">{([['PASSED', '通过'], ['PARTIAL', '部分掌握'], ['FAILED', '未通过']] as const).map(([value, label]) => <label key={value} className={`flex h-10 cursor-pointer items-center gap-2 rounded-md border px-3 text-sm ${attemptResult === value ? 'border-teal-300/50 bg-teal-300/10 text-teal-100' : 'border-white/10 text-zinc-300'}`}><Radio name="attempt-result" checked={attemptResult === value} onChange={() => setAttemptResult(value)} />{label}</label>)}</div></fieldset><label className="block text-sm text-zinc-400">复盘备注<Textarea value={attemptNote} onChange={(event) => setAttemptNote(event.target.value)} maxLength={2000} className="mt-1 min-h-20 px-3" placeholder="记录卡点、遗漏或下次注意事项" /></label><div className="flex flex-wrap gap-3"><Button type="button" variant="primary" disabled={pending} onClick={() => void saveAttempt()} className="h-10 px-3"><Save size={16} aria-hidden />{pending ? "保存中" : "保存本次作答"}</Button><Button type="button" variant="ghost" className="h-10 text-teal-300" onClick={() => setRevealed(false)}><Undo2 size={15} aria-hidden />返回修改</Button></div></div> : null}
      ...
      {revealed ? <>
        <section className="space-y-4 border-t border-white/10 pt-5" aria-labelledby="mistake-review-heading">
          ...
          {editing && !archived && !props.readOnly ? <div className="grid gap-3">
            <label className="text-sm text-zinc-400">标题<Input className="mt-1 px-3" value={title} onChange={(event) => setTitle(event.target.value)} /></label>
            <label className="text-sm text-zinc-400">题目正文<Textarea className="mt-1 min-h-32 px-3" value={questionText} onChange={(event) => setQuestionText(event.target.value)} /></label>
            <label className="text-sm text-zinc-400">来源<Input className="mt-1 px-3" value={source} onChange={(event) => setSource(event.target.value)} /></label>
            <label className="text-sm text-zinc-400">错因<Select className="mt-1 px-3" value={cause} onChange={(event) => setCause(event.target.value as MistakeCauseDto)}>{causeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label>
            <label className="text-sm text-zinc-400">错因补充说明<Textarea className="mt-1 min-h-20 px-3" value={causeNote} onChange={(event) => setCauseNote(event.target.value)} /></label>
            <label className="text-sm text-zinc-400">标准答案（可选）<Textarea className="mt-1 min-h-24 px-3" value={correctAnswer} onChange={(event) => setCorrectAnswer(event.target.value)} /></label>
            <label className="text-sm text-zinc-400">正确思路<Textarea className="mt-1 min-h-32 px-3" value={correctIdea} onChange={(event) => setCorrectIdea(event.target.value)} /></label>
            <EditorActionBar
