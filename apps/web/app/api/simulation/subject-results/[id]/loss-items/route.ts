...
import { idempotencyKeySchema } from "@/lib/study/schemas";
import { createSimulationLossItemCommand, listSimulationLossItems } from "@/lib/study/simulation-service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await createSimulationLossItemCommand(id, parsed.data, user.id), { status: 201 });
  } catch (error) {
    return apiErrorResponse(error);
  }
}
