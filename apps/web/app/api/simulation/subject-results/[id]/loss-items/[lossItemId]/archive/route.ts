...
import { archiveSimulationLossItem } from "@/lib/study/simulation-service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string; lossItemId: string }> }) {
  try {
    ...
    return NextResponse.json({ lossItem: await archiveSimulationLossItem(id, lossItemId, parsed.data.expectedRevision, user.id) });
  } catch (error) {
