...
import { archiveSimulationLossItemCommand } from "@/lib/study/simulation-service";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string; lossItemId: string }> }) {
  try {
    ...
    return NextResponse.json(await archiveSimulationLossItemCommand(id, lossItemId, parsed.data, user.id));
  } catch (error) {
