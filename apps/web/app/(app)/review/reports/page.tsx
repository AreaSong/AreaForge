...
export default async function ReviewReportsPage({
  ...
  const currentReportHref = `/review/reports?tab=current&period=${period}`;
  ...
  return (
    <PageFrame variant="dashboard-wide">
      ...
      <Toolbar label="报告视图与周期">
        <ReportFilter href={`/review/reports?tab=current&period=${period}`} active={tab === "current"}>当前报告</ReportFilter>
        <ReportFilter href={`/review/reports?tab=history&period=${period}`} active={tab === "history"}>历史回放</ReportFilter>
