...
export default async function ReviewScheduleDetailPage({ params }: { params: Promise<{ scheduleId: string }> }) {
  ...
  return (
    <article className="space-y-5">
      <Link className="text-sm text-teal-300 hover:underline" href="/knowledge/reviews">返回复习队列</Link>
      <header>
        <p className="text-sm text-teal-300">{schedule.targetType}</p>
        <h1 className="mt-1 text-2xl font-semibold text-white">复习排期详情</h1>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {schedule.status === "ACTIVE" ? <Link className="h-10 rounded-md border border-white/10 px-4 text-sm leading-10 text-teal-300" href={`/quick-review/${schedule.id}?returnTo=${encodeURIComponent(`/knowledge/reviews/${schedule.id}`)}`}>开始快速复习</Link> : null}
      </div>
