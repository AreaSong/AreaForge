          ...
          <ul className="divide-y divide-white/10 border-y border-white/10">
            {overview.recentEvidence.map((item) => (
              <li key={`${item.type}-${item.id}`} className="flex items-center justify-between gap-3 py-4">
                ...
                <Link href={withReturnTo(item.href, "/knowledge/overview")} className="grid size-9 shrink-0 place-items-center rounded-md text-teal-300 hover:bg-white/[0.06]" aria-label={`打开 ${item.title}`} title="打开证据"><ArrowRight size={16} aria-hidden /></Link>
              </li>
            ))}
          </ul>
