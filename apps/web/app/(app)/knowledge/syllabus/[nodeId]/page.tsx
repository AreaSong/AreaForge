...
export default async function SyllabusNodeDetailPage({ params }: { params: Promise<{ nodeId: string }> }) {
  ...
  return <SyllabusDetailClient node={node} parentOptions={parentOptions} schedule={schedules.find((item) => item.syllabusNodeId === node.id) ?? null} renderedAt={new Date().toISOString()} />;
}
