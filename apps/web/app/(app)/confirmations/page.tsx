...
export default async function ConfirmationsPage() {
  ...
  return (
    <PageFrame variant="dashboard-wide">
      <PageHeader eyebrow="确认中心" title="待确认" description="建议、报告和检验结果先停在这里，只有你确认后才会形成下一步动作。" action={<Link href="/confirmations/history" className="inline-flex h-10 items-center gap-2 rounded-md border border-white/10 px-3 text-sm text-zinc-200 hover:bg-white/5"><CheckCheck size={16} aria-hidden="true" />已处理记录</Link>} status={<Badge tone={items.length ? "warning" : "success"}>{items.length} 项待确认</Badge>} />
