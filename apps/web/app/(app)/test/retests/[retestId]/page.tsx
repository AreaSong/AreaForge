...
export default async function KnowledgeRetestDetailPage({ params, searchParams }: { params: Promise<{ retestId: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  return <PageFrame variant="content-focus"><KnowledgeRetestDetailClient initial={retest} userId={user.id} returnTo={returnTo} initialNow={new Date().toISOString()} /></PageFrame>;
}
