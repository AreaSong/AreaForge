  ...
  const nextAction = draft
    ...
    : !plan
      ...
      : activeMilestones.length > 0
        ...
        : { label: "查看周期趋势", href: "/stage/analytics?window=7", detail: "当前没有待确认建议或进行中里程碑，先用趋势判断是否需要调整。" };
  ...
  return (
    <section className="space-y-4 border-b border-white/10 pb-6" aria-labelledby="latest-stage-decision">
      ...
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
        ...
        {result.status === "applied" && inbox && inbox.createdCount > 0 ? <ButtonLink href={withReturnTo("/today/inbox", returnTo)} variant="primary"><ClipboardList size={15} aria-hidden="true" />处理入箱草稿</ButtonLink> : null}
        {result.status === "applied" && inbox && inbox.createdCount === 0 && inbox.reusedCount > 0 ? <ButtonLink href={withReturnTo("/today/inbox", returnTo)} variant="secondary"><ClipboardList size={15} aria-hidden="true" />查看计划收件箱</ButtonLink> : null}
      </div>
    </section>
  );
}
...
function stageOverviewHref(query: { createMilestone?: string; returnTo?: string }): string {
  ...
  return `/stage/overview${search ? `?${search}` : ""}`;
}
