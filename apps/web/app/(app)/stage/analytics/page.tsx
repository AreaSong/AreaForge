...
function WindowLink({ days, active }: { days: 7 | 30; active: boolean }) { return <Link href={`/stage/analytics?window=${days}`} aria-current={active ? "page" : undefined} className={`inline-flex h-9 items-center gap-2 rounded-md px-3 text-sm ${active ? "bg-white/10 text-white" : "text-zinc-400 hover:text-white"}`}><CalendarDays size={14} aria-hidden="true" />{days} 天</Link>; }
