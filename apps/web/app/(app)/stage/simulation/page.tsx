  ...
  return (
    <PageFrame variant="dashboard-wide">
      <PageHeader
        ...
        action={latestDraft
          ? <ButtonLink href={`/stage/simulation/${latestDraft.id}`} variant="primary"><ArrowRight size={16} />继续未完成模拟</ButtonLink>
...
function ExamRow({ exam, primary = false }: { exam: Exam; primary?: boolean }) {
  ...
  return (
    <Link href={`/stage/simulation/${exam.id}`} className={`group grid gap-3 border px-4 py-4 transition-colors sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center ${primary ? "border-teal-400/30 bg-teal-500/[0.06]" : "border-white/10 bg-white/[0.02] hover:border-white/20"}`}>
