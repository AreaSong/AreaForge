...
- 当前分支的 v1.4 AUTH 与 v1.5 RBAC 本地实施均已确认：账户安全、Workspace/Membership/Invitation/Selection、五级角色、对象 grant、Coach 建议确认链和脱敏 Operator 管理面已形成隔离 runtime、完整本地门禁和共享测试池桌面 spot check 证据；v1.6 数据任务/删除预览、v1.7 受控运维请求、v1.8 私有挑战/排名投影也已形成默认关闭的本地候选，并在一次性隔离 fixture 验证。敏感学习正文只允许资源 owner 或有效 grant 读取；候选 migration 尚未 apply 到共享测试库或生产，完整浏览器矩阵、PR/CI、Release 或生产证据仍缺。
