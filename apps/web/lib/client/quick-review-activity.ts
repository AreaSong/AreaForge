...
export function createQuickReviewActivityRuntime(
  ...
  function readHandoffClaim(userId: string): QuickReviewActivityClaim | null {
    const raw = environment.localStorage.getItem(claimKey(userId));
    return raw
  ...
  function createActivityLease(
    ...
    const lease: QuickReviewActivityLease = {
      ...
      markReleasing(commandId, action) {
        ...
        const key = claimKey(input.userId);
        const raw = environment.localStorage.getItem(key);
    ...
    const heartbeat = environment.setInterval(() => {
      const raw = environment.localStorage.getItem(claimKey(input.userId));
      if (!raw) {
  ...
  function writeClaim(claim: QuickReviewActivityClaim): void {
    environment.localStorage.setItem(claimKey(claim.userId), JSON.stringify(claim));
    environment.notifyClaimChange(claim.userId);
  }

  function removeOwnedClaim(claim: QuickReviewActivityClaim): void {
    const key = claimKey(claim.userId);
    const raw = environment.localStorage.getItem(key);
    if (!raw) return;
...
function createBrowserEnvironment(): QuickReviewActivityEnvironment {
  const requestExclusiveLock = typeof navigator !== "undefined" && navigator.locks
    ? async (
        name: string,
        options: LockRequestOptions,
        callback: (acquired: boolean) => Promise<void>,
  ...
  return {
    ...
    subscribeClaimChanges(userId, listener) {
      const onStorage = (event: StorageEvent) => {
        if (event.key === claimKey(userId)) listener();
      };
    ...
    subscribeMessages(userId, listener) {
      ...
      const onStorage = (event: StorageEvent) => {
        if (event.key !== messageKey(userId) || !event.newValue) return;
        const parsed = parseJson(event.newValue);
        if (parsed !== null) listener(parsed);
      };
      const onLocal = (event: Event) => listener((event as CustomEvent).detail);
      window.addEventListener("storage", onStorage);
      window.addEventListener(MESSAGE_EVENT_NAME, onLocal);
      return () => {
        channel?.close();
        window.removeEventListener("storage", onStorage);
        window.removeEventListener(MESSAGE_EVENT_NAME, onLocal);
      };
    },
