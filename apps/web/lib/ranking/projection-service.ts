...
import { prisma, getSafeRankingProjection, type Prisma } from "@areaforge/db";
...
import { throwRankingRebuildApiError } from "./rebuild-service";
import {
...
export async function getChallengeProjection(
  ...
  try { return await getSafeRankingProjection(prisma, actorId, challengeId); }
  catch (error) { return throwRankingRebuildApiError(error); }
}

/** 历史隔离 fixture 的兼容函数；HTTP 写入口已迁移到持久任务，不调用这里。 */
export async function rebuildChallengeProjection(
  ...
  requireRankingFeature({ multiUser: true, projection: true });
  if (process.env.RANKING_REBUILD_QUEUE_ENABLED === "true") throw new ApiError("RANKING_REBUILD_REQUIRES_JOB", 409);
