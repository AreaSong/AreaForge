  ...
  return (
    <PageFrame variant="dashboard-wide">
      ...
      <section className="grid gap-6 border-b border-white/10 pb-7 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)] lg:items-center">
        ...
        <ButtonLink href={primaryRisk ? riskHref(primaryRisk) : "/review/reports"} variant="primary" size="lg" className="w-full lg:w-auto">
...
function WindowLink({ days, active }: { days: 7 | 30; active: boolean }) { return <Link href={`/plan/stages/analytics?window=${days}`} aria-current={active ? "page" : undefined} className={`inline-flex h-9 items-center gap-2 rounded-md px-3 text-sm ${active ? "bg-white/10 text-white" : "text-zinc-400 hover:text-white"}`}><CalendarDays size={14} aria-hidden="true" />{days} 天</Link>; }
