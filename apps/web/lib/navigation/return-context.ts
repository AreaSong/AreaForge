const RETURN_CONTEXTS: ReadonlyArray<{ prefix: string; label: string }> = [
  ...
  { prefix: "/roadmap/allocation/drafts", label: "返回收件箱" },
  { prefix: "/roadmap/allocation/tasks", label: "返回任务详情" },
  { prefix: "/roadmap/stages", label: "返回阶段概览" },
  { prefix: "/roadmap/allocation", label: "返回投入安排" },
  ...
  { prefix: "/roadmap/reviews/daily", label: "返回每日复盘" },
  { prefix: "/roadmap/reviews", label: "返回周期复盘" },
...
export function getReturnContextLabel(returnTo: string | undefined, fallbackLabel: string): string {
  ...
  if (pathname === "/roadmap/reviews/daily") return "返回复盘";
