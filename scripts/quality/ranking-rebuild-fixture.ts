...
export const RANKING_SCHEMA_SHA256 = "ab77429b40205a644f13cdf03864f9b73aa45b03003cd4161763bccf33a5a7e0";
export interface RankingRebuildFixture {
...
export function loadRankingFixture(root: string, repository = process.cwd()): RankingRebuildFixture {
  ...
  if (uid === undefined || uid === 0 || gid === undefined || !path.isAbsolute(root) || root !== realpathSync(root)
    ...
    || !/^areaforge-v20-ranking-[A-Za-z0-9]+$/.test(path.basename(root))) invalid();
  ...
  const marker = readRankingPrivate(path.join(root, ".areaforge-ranking-fixture.json"), uid);
  const secret = readRankingPrivate(path.join(root, ".fixture.private.json"), uid);
  ...
  if (Object.keys(marker).length !== keys.length || keys.some(key => !(key in marker)) || marker.schemaVersion !== 1
    || marker.fixtureKind !== "ranking-rebuild" || marker.ownerUid !== uid || marker.ownerGid !== gid
    || marker.repositoryHash !== createHash("sha256").update(realpathSync(repository)).digest("hex")) invalid();
  const suffix = String(marker.databaseName).match(/^areaforge_v20_ranking_([a-f0-9]{12})$/)?.[1];
  if (!suffix || marker.containerName !== `areaforge-v20-ranking-${suffix}` || marker.volumeName !== `areaforge-v20-ranking-${suffix}-data`
  ...
  return { root, databaseName: String(marker.databaseName), containerName: String(marker.containerName), volumeName: String(marker.volumeName),
    port: Number(marker.port), image: String(marker.image), ownerUid: uid, ownerGid: gid, operatorEmail: `operator-ranking-${suffix}@example.test`,
...
export function rankingFixtureEnvironment(fixture: RankingRebuildFixture): NodeJS.ProcessEnv {
  return { ...rankingToolEnvironment(), DATABASE_URL: `postgresql://fixture:${fixture.password}@127.0.0.1:${fixture.port}/${fixture.databaseName}`,
    DOTENV_CONFIG_PATH: path.join(fixture.root, ".fixture-env-disabled"), AREAFORGE_RANKING_REBUILD_ISOLATED_DB: "1",
    AREAFORGE_RANKING_REBUILD_FIXTURE_ROOT: fixture.root, AUTH_ADMIN_EMAIL: fixture.operatorEmail,
    ...
    RANKING_ENABLED: "true", RANKING_PROJECTION_ENABLED: "true", RANKING_REBUILD_QUEUE_ENABLED: "true", DATA_JOB_WORKER_ENABLED: "true",
...
export function rankingToolEnvironment(): NodeJS.ProcessEnv {
  return Object.fromEntries(["PATH", "HOME", "TMPDIR", "PNPM_HOME", "LANG", "LC_ALL", "DOCKER_HOST", "DOCKER_CONTEXT"]
    .flatMap(key => process.env[key] ? [[key, process.env[key]]] : []));
}

export function readRankingPrivate(file: string, uid: number): Record<string, unknown> {
...
export async function verifyRankingFixtureLedger(client: PrismaClient, fixture: RankingRebuildFixture): Promise<number> {
  ...
  assert.equal(createHash("sha256").update(readFileSync("prisma/schema.prisma")).digest("hex"), RANKING_SCHEMA_SHA256);
...
export function assertRankingFixtureContainer(fixture: RankingRebuildFixture): void {
  const env = rankingToolEnvironment();
  ...
  if (row.Config.Labels?.["com.areaforge.fixture"] !== "ranking-rebuild" || row.Config.Labels?.["com.areaforge.fixture.owner"] !== String(fixture.ownerUid)
  ...
  if (volume.Labels?.["com.areaforge.fixture"] !== "ranking-rebuild" || volume.Labels?.["com.areaforge.fixture.owner"] !== String(fixture.ownerUid)) invalid();
...
function invalid(): never { throw new Error("RANKING_REBUILD_FIXTURE_INVALID"); }
