...
import { loadSearchIndexFixture, searchIndexFixtureEnvironment, verifySearchIndexFixtureLedger } from "./search-index-fixture";

async function main() {
  const fixture = loadSearchIndexFixture(process.env.AREAFORGE_SEARCH_INDEX_FIXTURE_ROOT ?? "");
  const env = searchIndexFixtureEnvironment(fixture); Object.assign(process.env, env);
  const jobId = process.env.SEARCH_FIXTURE_JOB_ID; const point = process.env.SEARCH_FIXTURE_PAUSE_AT;
  const leaseMs = Number(process.env.SEARCH_FIXTURE_LEASE_MS ?? 30_000);
  ...
  try {
    await verifySearchIndexFixtureLedger(client, fixture);
    const row = await client.dataJob.findUniqueOrThrow({ where: { id: jobId! } });
    assert.equal(row.kind, "SEARCH_INDEX_REBUILD"); assert.ok(row.workspaceId);
    const handler = createWorkspaceSearchHandler(client, env);
    await runDataJobWorker({ enabled: true, client, workerId: `search-child-${process.pid}`, leaseMs,
      ...
      handlers: [{ kind: "SEARCH_INDEX_REBUILD", prepare: async context => {
