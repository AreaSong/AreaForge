...
import { searchIndexToolEnvironment } from "./search-index-fixture";

async function main() {
  const root = process.argv[2] ?? ""; const uid = process.getuid!();
  if (!path.isAbsolute(root) || root !== realpathSync(root) || uid === 0 || !/^areaforge-v20-search-[A-Za-z0-9]+$/.test(path.basename(root))) throw new Error("ROOT_INVALID");
  ...
  const suffix = randomBytes(6).toString("hex"); const containerName = `areaforge-v20-search-${suffix}`;
  const databaseName = `areaforge_v20_search_${suffix}`; const volumeName = `${containerName}-data`;
  ...
  run(["volume", "create", "--label", "com.areaforge.fixture=search-index", "--label", `com.areaforge.fixture.owner=${uid}`, volumeName]);
  run(["run", "-d", "--name", containerName, "--label", "com.areaforge.fixture=search-index", "--label", `com.areaforge.fixture.owner=${uid}`,
  ...
  const marker = { schemaVersion: 1, fixtureKind: "search-index", databaseName, containerName, volumeName, port: Number(port.split(":")[1]), image,
    ownerUid: uid, ownerGid: process.getgid!(), repositoryHash: createHash("sha256").update(realpathSync(process.cwd())).digest("hex") };
  writeFileSync(path.join(root, ".areaforge-search-fixture.json"), JSON.stringify(marker), { flag: "wx", mode: 0o600 });
...
main().catch(() => { console.error("SEARCH_INDEX_FIXTURE_CREATE_FAILED; preserve registered resources"); process.exitCode = 1; });
