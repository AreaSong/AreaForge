...
import type { V11JourneyEvidence } from "./v11-browser-evidence-contract";
import { createSyntheticJourneyIdentity, writeSyntheticJourneyEvidence } from "./product-experience-selftest-fixture";
...
function writeJourneyEvidence(file: string, screenshotDir: string, generatedAt: string): void {
  writeSyntheticJourneyEvidence({ root, file, screenshotDir, generatedAt, ...binding });
}

function productionRuntimeIdentityAt(
  observedAt: string,
): V11JourneyEvidence["runtimeIdentityEvidence"]["runtimeIdentity"] {
  return createSyntheticJourneyIdentity({ ...binding, observedAt });
}
