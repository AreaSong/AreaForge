...
export const QUOTA_SCHEMA_SHA256 = "6001f7ef0e030295a589f4e845eba3f75ba3b214885863f9251e637aa4ae4583";
...
export function loadQuotaFixture(root: string, repository = process.cwd()): QuotaFixture {
  ...
  if (uid === undefined || uid === 0 || gid === undefined || !path.isAbsolute(root) || root !== realpathSync(root)
    ...
    || !/^areaforge-v20-quota-[A-Za-z0-9]+$/.test(path.basename(root))) invalid();
  ...
  const marker = readQuotaPrivate(path.join(root, ".areaforge-quota-fixture.json"), uid);
  const secret = readQuotaPrivate(path.join(root, ".fixture.private.json"), uid);
  ...
  if (Object.keys(marker).length !== keys.length || keys.some(key => !(key in marker)) || marker.schemaVersion !== 1
    || marker.fixtureKind !== "quota" || marker.ownerUid !== uid || marker.ownerGid !== gid
    || marker.repositoryHash !== createHash("sha256").update(realpathSync(repository)).digest("hex")) invalid();
  const suffix = String(marker.databaseName).match(/^areaforge_v20_quota_([a-f0-9]{12})$/)?.[1];
  if (!suffix || marker.containerName !== `areaforge-v20-quota-${suffix}` || marker.volumeName !== `areaforge-v20-quota-${suffix}-data`
  ...
  return { root, databaseName: String(marker.databaseName), containerName: String(marker.containerName), volumeName: String(marker.volumeName),
    port: Number(marker.port), image: String(marker.image), ownerUid: uid, ownerGid: gid, operatorEmail: `operator-quota-${suffix}@example.test`,
...
export const QUOTA_FIXTURE_LIMITS = { maxActiveJobs: "2", maxExports24h: "3" } as const;
export function quotaFixtureEnvironment(fixture: QuotaFixture): NodeJS.ProcessEnv {
  return { ...quotaToolEnvironment(), DATABASE_URL: `postgresql://fixture:${fixture.password}@127.0.0.1:${fixture.port}/${fixture.databaseName}`,
    DOTENV_CONFIG_PATH: path.join(fixture.root, ".fixture-env-disabled"), AREAFORGE_DATA_JOB_QUOTA_ISOLATED_DB: "1",
    AREAFORGE_DATA_JOB_QUOTA_FIXTURE_ROOT: fixture.root, AUTH_ADMIN_EMAIL: fixture.operatorEmail,
    ...
    DATA_JOB_QUOTA_ENABLED: "true", DATA_JOB_QUOTA_MAX_ACTIVE_JOBS: QUOTA_FIXTURE_LIMITS.maxActiveJobs,
    DATA_JOB_QUOTA_MAX_EXPORTS_24H: QUOTA_FIXTURE_LIMITS.maxExports24h,
...
export function quotaToolEnvironment(): NodeJS.ProcessEnv {
  return Object.fromEntries(["PATH", "HOME", "TMPDIR", "PNPM_HOME", "LANG", "LC_ALL", "DOCKER_HOST", "DOCKER_CONTEXT"]
    .flatMap(key => process.env[key] ? [[key, process.env[key]]] : []));
}

export function readQuotaPrivate(file: string, uid: number): Record<string, unknown> {
...
export async function verifyQuotaFixtureLedger(client: PrismaClient, fixture: QuotaFixture): Promise<number> {
  ...
  assert.equal(createHash("sha256").update(readFileSync("prisma/schema.prisma")).digest("hex"), QUOTA_SCHEMA_SHA256);
...
export function assertQuotaFixtureContainer(fixture: QuotaFixture): void {
  ...
  if (row.Config.Labels?.["com.areaforge.fixture"] !== "quota" || row.Config.Labels?.["com.areaforge.fixture.owner"] !== String(fixture.ownerUid)
  ...
  if (volume.Labels?.["com.areaforge.fixture"] !== "quota" || volume.Labels?.["com.areaforge.fixture.owner"] !== String(fixture.ownerUid)) invalid();
...
function invalid(): never { throw new Error("QUOTA_FIXTURE_INVALID"); }
