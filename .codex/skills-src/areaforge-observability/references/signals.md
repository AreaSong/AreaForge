...
Use `pnpm incident:index` and `pnpm incident:index:validate docs/development/incident-index.json` for the deterministic metadata-only projection of both active records (`open`, `mitigated`, `follow-up`) and resolved records; resolved records enter the resolved group only with `postIncidentReview=yes`. The index does not represent live incident state and cannot prove current production health, containment, or residual closure.
