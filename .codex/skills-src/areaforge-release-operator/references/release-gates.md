...
- Supply-chain evidence record when closing or reviewing `AF-RISK-SC-001` / `AF-RISK-SC-002`: copy `docs/development/release-supply-chain-record-template.md`, configure both the record and Release assets for `pnpm sc:sc-002:preflight`, and run `pnpm release:supply-chain:validate <record> <release-assets-dir> --strict`. Use `pnpm release:supply-chain:record <release-assets-dir>` to generate a redacted draft first. Record-only validation is structural evidence and cannot prove signed Release readiness.
