...
- Ops readiness or residual ledger: `pnpm ops:readiness`, `pnpm ops:readiness:summary`, `pnpm ops:evidence:bundle`, `pnpm ops:alert:preview`, `pnpm smoke:prod-readonly:selftest`, `pnpm smoke:prod-readonly:config:selftest`, `pnpm smoke:prod-readonly:record:selftest`, `pnpm alert:drill:selftest`, `pnpm docs:readiness`, `git diff --check`; add release/updater gates when release workflow or updater trust changed.
- Long-term operability control plane: `pnpm enterprise:operability:preflight`, `pnpm maintenance:cadence:preflight`, `pnpm release:train:preflight`, `pnpm ops:readiness`, `pnpm residuals:validate`, `pnpm residuals:review-due`, `pnpm docs:readiness`, `pnpm skills:validate`, `git diff --check`.
- Maintenance, incident, restore drill, or update-agent status records: run the matching selftest (`pnpm maintenance:window:selftest`, `pnpm incident:record:selftest`, `pnpm restore:drill:selftest`, or `pnpm update-agent:status:selftest`), then docs/ops gates based on the changed path. If a real record is added, validate it with the matching `*:validate` script.
...
- Release supply-chain evidence: `pnpm release:supply-chain:selftest`, `pnpm release:supply-chain:record:selftest`, `pnpm github-release-updater:preflight`, `pnpm governance:preflight`, and `git diff --check`; if a real record is generated, also run `pnpm release:supply-chain:validate <record>`.
