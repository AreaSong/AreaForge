...
- OPS-001 evidence preflight: after redacted smoke record, update-agent status record, and operational evidence bundle are copied locally, run `pnpm ops:ops-001:preflight` with `AREAFORGE_OPS001_SMOKE_RECORD`, `AREAFORGE_OPS001_UPDATE_STATUS_RECORD`, `AREAFORGE_OPS001_EVIDENCE_BUNDLE`, and optionally `AREAFORGE_OPS001_CLOSURE_PACKET`. It is read-only and reports `needs_evidence`, `ready_to_generate_packet`, `ready_for_human_close`, or `invalid`.
- OPS-001 closure packet: after smoke record, update-agent status record, and operational evidence bundle all pass their validators, generate `pnpm ops:ops-001:closure <smoke-record> <update-status-record> <evidence-bundle> > <packet>` and validate it with `pnpm ops:ops-001:closure:validate <packet>` before proposing residual ledger closure.
