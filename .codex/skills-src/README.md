...
- 维护窗口、事故、恢复演练、update-agent redacted status 和 OPS-001 收口包分别使用 `pnpm maintenance:window:validate`、`pnpm incident:record:validate`、`pnpm restore:drill:validate`、`pnpm update-agent:status:validate` 和 `pnpm ops:ops-001:preflight` / `pnpm ops:ops-001:closure:validate`。
