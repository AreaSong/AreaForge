...
> 当前实现覆盖本文件历史规划之外的最终导航收口：登录默认 `/focus`；一级导航为开始学习、今日、计划、知识、检验、复盘、确认中心、设置；开始学习没有二级导航；一级/二级侧栏可分别折叠；三级内容页不重复导航；面包屑右侧提供 AI 与页面功能；底部共享工具栏显示活动计时、上一页面、同步及跨设备状态。本文中仍保留的 `/today/plan`、`/stage/*` 等路径表属于历史规划快照，不是当前 canonical 路由；当前路由以 `docs/ux/site-navigation.md` 和 `apps/web/lib/navigation/batch7.ts` 为准。
