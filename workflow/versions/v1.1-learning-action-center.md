...
- 计划状态：**S0–S5 已形成 `v1.1.0` Release 并进入生产；Batch 11 当前继续收拢发布后产品化修复。生产数据清理和 22 个静态入口复验已完成，本地修复尚未形成新 commit、tag、Release 或 production apply**。
...
- Batch 11 修复候选边界：生产清理与 22 个入口复验已经完成，但当前本地工作树尚未 commit，修复 Release admission 仍为 `NOT READY`。本地门禁不授权 push、tag、GitHub Release、production backup/migration/apply/smoke/rollback、自动应用策略变化或 residual 关闭。
...
- 2026-07-31 已按精确高风险确认完成生产数据收口：数据库与上传目录备份校验通过；单事务将活动科目 `12 -> 7`，迁移新高等数学中的 1 次真实 session 和 1 张真实 note，保留并重命名 legacy 数学与四个 408 科目，删除 5 个重复科目；同时精确删除已盘点 `[AF_SMOKE]` 业务数据、22 条对应审计事件、2 条附件 metadata 和 2 个已备份合成文件。清理后目标 ID、marker、审计事件均剩余 0，12 项只读完整性聚合与附件双向一致性检查通过。
