...
- 计划状态：**S0–S5 已形成 `v1.1.0` Release 并进入生产；Batch 11 当前以 `v1.1.1` 收拢发布后产品化修复候选。生产数据清理和 22 个静态入口复验已完成，但尚未 push、创建 `v1.1.1` Release 或 production apply**。
...
- 当前版本状态：生产与稳定 GitHub Release 基线为 `v1.1.0` / commit `4dbdb31a96498487af09aa7f90275bfc549448f3`；隔离分支 `codex/v1.1-learning-action-center` 的 package version 为 `1.1.1`，包含尚未发布的首次设置、科目管理和核心页面产品化修复。
- 当前可用基线：公网 health 已只读验证 production `1.1.0` 和 verified runtime identity；`v0.1.9` 及其 OPS/供应链 closeout 只保留历史证据。
...
- 当前实现边界：`v1.1.0` 的历史冻结证据只绑定当时 source commit；本轮发布后修复改变了 Web 源和体验 fingerprint，必须冻结新的 `v1.1.1` source commit 后重采 compatibility/browser、current-bound completion、SC-002/SC-004，才能进入修复 Release admission。
- Batch 11 修复候选边界：生产清理与 22 个入口复验已经完成；package/admission/workflow 已面向 `v1.1.1` 修复候选更新，但 current-bound browser/compatibility、SC-002/SC-004 和 Release admission 尚未重采完成，`v1.1.1` admission 仍为 `NOT READY`。本地门禁不授权 push、tag、GitHub Release、production backup/migration/apply/smoke/rollback、自动应用策略变化或 residual 关闭。
...
| 设置 | `/settings/ai` | 当前浏览器外部 Provider 开关、服务端配置状态和 payload 隐私边界 |
...
- 真实外呼必须同时满足当前浏览器偏好开启、`AI_ENABLED=true`、Provider 配置完整，以及各路径既有的最小 payload/schema/preview/限流条件。任一条件不满足都回退本地规则；偏好不会绕过 `AI_PAYLOAD_BINDING_SECRET`、四类草稿发送前预览或长期阶段最小聚合字段。
...
- 后续顺序固定为：冻结 `v1.1.1` 修复 commit 并重采当前绑定 browser/compatibility、SC-002/SC-004 证据 -> `v1.1.1` Release 确认 -> 独立 production apply 确认。任一步都不自动关闭 residual，也不改变 `AREAFORGE_AUTO_APPLY=none`。
