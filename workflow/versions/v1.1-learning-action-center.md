...
- 计划状态：**S0–S5 已形成 `v1.1.0` Release 并进入生产；Batch 11 当前继续收拢发布后产品化修复。生产数据清理和 22 个静态入口复验已完成，本地修复已形成新的 checkpoint，但尚未 push、创建修复 Release 或 production apply**。
...
- 当前版本状态：生产与稳定 GitHub Release 基线为 `v1.1.0` / commit `4dbdb31a96498487af09aa7f90275bfc549448f3`；隔离分支 `codex/v1.1-learning-action-center` 当前包含尚未发布的首次设置、科目管理和核心页面产品化修复。
...
- 当前实现边界：`v1.1.0` 的历史冻结证据只绑定当时 source commit；本轮发布后修复改变了 Web 源和体验 fingerprint，必须冻结新 commit 后重采 compatibility/browser、current-bound completion、SC-002/SC-004，才能进入修复 Release admission。
- Batch 11 修复候选边界：生产清理与 22 个入口复验已经完成，本地修复已有 checkpoint，但 current-bound browser/compatibility、SC-002/SC-004 和 Release admission 尚未重采完成，修复 Release admission 仍为 `NOT READY`。本地门禁不授权 push、tag、GitHub Release、production backup/migration/apply/smoke/rollback、自动应用策略变化或 residual 关闭。
...
- 后续顺序固定为：冻结修复 commit 并重采当前绑定 browser/compatibility、SC-002/SC-004 证据 -> 修复 Release 确认 -> 独立 production apply 确认。任一步都不自动关闭 residual，也不改变 `AREAFORGE_AUTO_APPLY=none`。
