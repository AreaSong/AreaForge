...
- Open questions：产品主流程已定案；实施前仍必须完成源文档逐项同步、数据生命周期确认、依赖准入、兼容 floor、任务拆分、`AF-RISK-OPS-007`、完整产品数据 migration 包和各次生产 apply 的实施确认。`AF-RISK-OPS-006` 已达到隔离 PostgreSQL `local_verified`，但 matching signed patch 与 independent production apply 仍是前置门禁。
...
- Residual risk IDs：`AF-RISK-UX-001`、`AF-RISK-OPS-006`、`AF-RISK-OPS-007`、`AF-RISK-SC-002`、`AF-RISK-SC-004`，以及实施前必须登记的 `AF-RISK-DATA-001`（学习树规范化 Markdown 长期留存、备份扩散与未来撤销/物理删除缺口）。SC-002 当前以 exact commit `5bec626` 的 CI-only 证据关闭，但任何 v1.1 Release 必须重新生成匹配该 Release commit 的证据；SC-004 的 main 保护/readback/受控 PR 基线已验证，residual 仍待人工决策，后续 Release 前必须重新读回确认未漂移。DATA-001 必须记录 data owner、验证 owner、接受期限、备份副本处理、用户撤销/未来删除路线和关闭条件，未完成登记与人工接受时不得开放导入 confirm。
