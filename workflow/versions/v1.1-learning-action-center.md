...
- 当前工作树已修复接管 stable key 冲突、重复默认科目、无 ACTIVE workspace 引导，补齐科目与分组编辑/相邻排序/归档/恢复，并收敛 App Shell、今日、计划、知识、复盘、阶段和设置的信息架构。既有完整 24 migration 隔离库上的 M1-M3 runtime 已按最新源码重跑通过，覆盖 archived-group 拒绝、相邻换位、边界 no-op、revision 和审计门禁；全量 `pnpm check` 已通过，九个核心入口在 `1440x900` / `390x844` 的当前源码快速验收为 18/18，且科目换位与归档分组编辑回退已实测。该快速验收不替代冻结 commit 后的正式 browser evidence runner/validator。
