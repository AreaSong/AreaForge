...
- 计划状态：**S0–S2 与 Batch 3–10 已落地；Batch 11 的 pre-freeze 本地候选已通过 24-migration compatibility floor、production-mode 18 条旅程、24 项无障碍检查与 `pnpm check`；本文件所在提交冻结为目标 source commit，后续只允许按该提交重采 compatibility/browser、补齐 current-bound 完成记录与 Release admission 证据**。
...
- 当前版本状态：产品、页面信息架构、学习树协议和关联画布已定案，功能实现与证据 runner 已冻结在本文件所在 source commit；隔离分支 `codex/v1.1-learning-action-center` 后续只允许契约白名单内的 evidence-only 收口。
- 当前可用基线：生产与仓库签名 Release 均为 `v0.1.9`（冻结 commit `749692ba719d801f14186a94af97b96350380141`）；第一版与第二阶段现有文档范围内的核心能力已在该基线上运行。
- 是否已有线上 v1.1 Release：没有。`v0.1.9` 是本计划的实现与运维基线，不是 v1.1 产品完成证据。
...
- 当前实现边界：Batch 3–10 与 Batch 11 补缺代码已在隔离分支形成；结构化浏览器 runner/validator/admission binding 已完成 selftest，pre-freeze dirty worktree 本地 production-build 候选也曾用 19 个隔离合成账户通过 9×desktop/mobile 旅程与 24 项独立无障碍证据。该历史证据不绑定本文件所在 source commit，必须在新空库重采后才能计入 current-bound 完成和 Release admission；它不表示已创建完整 minor Release、执行生产 migration/apply/updater、完成生产 smoke，或关闭任何 residual。
- Batch 11 候选边界：本地版本已统一为 `1.1.0`，本文件所在提交是冻结 source commit；冻结提交的 compatibility/browser、current-bound completion、SC-002/SC-004 尚待重采，因此 complete minor Release admission 仍为 `NOT READY`。候选记录与本地门禁不授权 tag、GitHub Release、production backup/migration/apply/smoke/rollback、自动应用策略变化或 residual 关闭。
