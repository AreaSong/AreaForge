...
- 计划状态：**S0–S2 与 Batch 3–10 已落地；Batch 11 正按本文件逐功能重验、补缺并重采 24-migration compatibility floor 与 production-mode 九条旅程，尚未形成 current-bound 本地完成记录**。
...
- 当前版本状态：产品、页面信息架构、学习树协议和关联画布已定案，功能实现正在做最终全局复验；隔离分支 `codex/v1.1-learning-action-center` 承载尚未提交且仍会变化的本地候选。
...
- 当前实现边界：Batch 3–10 与 Batch 11 补缺代码已在隔离分支形成；结构化浏览器 runner/validator/admission binding 已完成 selftest，但真实 9×desktop/mobile 旅程与 24 项独立无障碍证据仍待经确认在隔离环境重采。只有新鲜通过的静态、runtime、浏览器和证据绑定门禁才能计入最终完成；这不表示已创建完整 minor Release、执行生产 migration/apply/updater、完成生产 smoke，或关闭任何 residual。
- Batch 11 候选边界：本地版本已统一为 `1.1.0`，当前工作树尚未冻结且完成记录无 current fingerprint；complete minor Release admission 为 `NOT READY`。SC-002/SC-004 只能在未来目标 commit 冻结后按同一 commit 重采。候选记录与本地门禁不授权 commit、push、tag、GitHub Release、production backup/migration/apply/smoke/rollback、自动应用策略变化或 residual 关闭。
