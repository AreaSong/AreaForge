...
> 当前实现覆盖本文件历史规划之外的最终导航收口：登录默认 `/focus`；一级导航为开始学习、今日、知识、检验、路线；设置位于侧栏底部工具区，确认中心和 AI 属于公共工具栏。开始学习只允许 `STUDY` 自由学习；专项复测和模拟考试分别在检验工作台配置后启动 `REVIEW`/`TEST` 专用计时，结束后必须进入 `CLOSING` 并完成来源页收口。一级/二级侧栏可分别折叠；三级内容页不重复导航；面包屑右侧提供页面功能；底部共享工具栏显示来源页面、同步及跨设备状态。本文其余旧路径表和历史批次记录仅用于保留规划/证据上下文，不是当前 canonical 路由；当前路由以 `docs/ux/site-navigation.md` 和 `apps/web/lib/navigation/batch7.ts` 为准。知识卡片 canonical 路由为 `/knowledge/cards`，`/knowledge/notes` 不再作为入口。
