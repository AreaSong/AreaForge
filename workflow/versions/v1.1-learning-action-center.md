...
> 当前实现覆盖本文件历史规划之外的最终导航收口：登录默认 `/focus`；一级导航为开始学习、今日、知识、检验、路线；设置位于侧栏底部工具区，确认中心和 AI 属于公共工具栏。开始学习只允许 `STUDY` 自由学习；专项复测和模拟考试分别在检验工作台配置后启动 `REVIEW`/`TEST` 专用计时，结束后必须进入 `CLOSING`，收口由公共固定窗口承载，可在任意页面继续并按来源上下文返回。一级/二级侧栏可分别折叠；三级内容页不重复导航；面包屑右侧提供页面功能；底部共享工具栏显示来源页面、同步及跨设备状态。本文其余旧路径表和历史批次记录仅用于保留规划/证据上下文，不是当前 canonical 路由；当前路由以 `docs/ux/site-navigation.md`、`apps/web/lib/navigation/canonical-routes.ts` 和 `apps/web/lib/navigation/app-navigation.ts` 为准。知识卡片 canonical 路由为 `/knowledge/cards`，`/knowledge/notes` 不再作为入口。

## 当前状态

- 计划状态：**S0–S5 已完成；`v1.1.0` 与 Batch 11 发布后产品化修复 `v1.1.1` 均已形成稳定 GitHub Release 并完成受控 production apply。生产数据清理、22 个静态入口复验和更新后健康校验均已完成**。
- 执行大阶段（对人）：`S0 基线对齐` → `S1 Batch0 文档` → `S2 Batch1–2 门禁复核` → `S3 Batch3–6 数据与服务` → `S4 Batch7–10 可见产品` → `S5 Batch11 完整 minor 发布`。内部仍严格使用 Batch 0–11 编号，不另起批次体系。
- 当前版本状态：本计划范围的最新稳定补丁为 `v1.1.2`；仓库最新稳定 GitHub Release 已进入 `v1.2.0` / commit `018cdfaa7a58cea2b32a33acaa0b968f29b9e09a`，其 workflow、current-bound browser、accessibility、compatibility、SC-002、SC-004 与签名资产严格校验均已通过。
- 归档生产状态：交付与回滚记录基线为 `v1.1.1` / commit `f995310e30c41270ee1e0a1c1ceeae9b6a8017eb`；`v1.1.2` 发布记录未执行 production apply。新鲜公网 health 已观测到 `v1.2.0`，完整服务器证据仍待核验，详见 operational-readiness。
- 历史可用证据：当时公网 health 报告 production `1.1.1` / verified，Web/PostgreSQL healthy、migration 24/24、health/extra smoke PASS、journal clean。该记录与 `v0.1.9`、`v1.1.0` 的 OPS/供应链 closeout 均不能证明当前生产健康。
- 是否已有线上 v1.1 Release：有。`v1.1.0` annotated tag 与稳定 GitHub Release 已创建并完成 production apply；tag 本身没有 GPG signature，签名事实只适用于 Release 资产。
- Batch 1/2 缩减规则（相对原文「独立 OPS-006/007 patch 发布」）：`AF-RISK-OPS-006` / `AF-RISK-OPS-007` 已在 `v0.1.9` 达到 `closed-evidence`（含 matching signed Release 与 independent production apply 证据）。S2 只做四级 gate 复核与缺项补齐；**不重复实施** concurrency / attachment staging，也不另开 OPS-006/007 patch Release，除非复核证明证据不足。完整产品仍走 Batch 11 的 complete minor Release；14.4 前两段地基 Release 记为「已由 `v0.1.9` 满足」。
- 当前记录用途：汇总旧讨论任务 `019f6592-46db-7d71-8b30-d90b92acccf6`、当前任务以及后续多轮闭环审查中已经确认的产品、页面、交互、学习树模板、关联画布、状态反馈、数据、API、迁移、验证和发布决策。
- 当前实现边界：`v1.1.0` 的历史冻结证据只绑定当时 source commit；发布后修复已按新 commit 重采 compatibility/browser、current-bound completion、SC-002/SC-004，并通过 Release admission。
- Batch 11 Release 边界：生产清理与 22 个入口复验、受保护 PR #27/#28、`v1.1.1` tag、稳定 GitHub Release、签名 Release 资产校验和 Web 受控 production apply 均已完成。更新流程创建并校验数据库、上传目录与环境配置备份，migration 仍为 24/24，health/extra smoke PASS，未执行 restore、rollback、自动应用策略变化或 residual 关闭。
- 文档同步边界：Batch 0 必须同步 Exact docs 清单中的 `docs/product/**`、`docs/architecture/**`、`docs/modules/**`、`docs/ux/**`、`docs/security/**`、`docs/development/**` 和 `tasks/**`；同步时必须区分「规划 / 未实现」与「当前已实现」，不得扩大 docs 100% 完成声明。
- 历史证据边界：v1.1 不计入既有 docs 100% 完成声明，也不改写 v0.1-v1.0、Package A-E 或生产 `v0.1.7`/`v0.1.9` 的历史完成事实。
- 工作区边界：在隔离分支 `codex/v1.1-learning-action-center` 推进；不与未收口的无关 dirty worktree 混提。
...
- Open questions：产品主流程已定案；S1 须完成源文档逐项同步、`AF-RISK-DATA-001` 登记、依赖准入预告、兼容 floor 说明、任务拆分与完整产品数据 migration 确认包骨架。S2 须复核 OPS-006/007 四级 gate 是否已被 `v0.1.9` 证据覆盖。业务代码实施前仍须取得各 Batch 对应高风险确认。
- Decisions：五工作台、当前考试工作区、自定义科目与 408 分组、科目快捷计时、页面职责和信息密度、五个全局状态灯、状态栏/Popover/Drawer/Modal/Toast 分工、桌面分栏与移动全屏详情、稳定路由与返回上下文、学习树模板协议、导入版本与原子合并、全局关联画布、知识卡片、任务依赖、今日推荐、空队列、全屏专注、低转化补救、统一复习、`CheckIn v2`、恢复三阶、复习任务桥接、计划收件箱、资料资产、动机内容库、浏览器前台通知、模拟分科总分与结构化失分、报告/阶段不可逆版本决策、错误恢复、迁移拆分和三段发布均已定案。另定：Batch 1/2 在 `v0.1.9` 证据充分时缩减为门禁复核。
...
- Validation profile：Mission-Critical + Product Experience。S0/S1 为 workflow/docs-only；实现阶段包含 Web、core、db、Prisma migration、storage、桌面/移动浏览器和临时 PostgreSQL 验证。
...
- Source baseline：生产 `v0.1.9`、当前 Prisma schema、现有 API/UI、Package A-E 与 docs 100% 当前范围完成证据，以及本文件记录的后续产品讨论。历史对照基线仍可引用 `v0.1.7` 回滚目标 digest，但不得把 `v0.1.7` 当作当前生产版本。
- Residual risk IDs：`AF-RISK-UX-001`、`AF-RISK-OPS-006`、`AF-RISK-OPS-007`、`AF-RISK-SC-002`、`AF-RISK-SC-004`（后四项当前多为 `closed-evidence`，v1.1 Release 前仍须按触发条件重开或重采），以及 Batch 0 必须登记的 `AF-RISK-DATA-001`（学习树规范化 Markdown 长期留存、备份扩散与未来撤销/物理删除缺口）。SC-002 任何 v1.1 Release 必须重新生成匹配该 Release commit 的证据；SC-004 每次 Release 前须 fresh readback。DATA-001 必须记录 data owner、验证 owner、接受期限、备份副本处理、用户撤销/未来删除路线和关闭条件，未完成登记与人工接受时不得开放导入 confirm。
...
- Apply boundary：S0/S1 只是仓库文档与任务变更，不改变运行时能力；未来普通读取为 R0，本地临时库和 fixture 为 R1，用户显式创建/确认为 R2，生产 migration、备份、恢复或 updater apply 仍是独立 R4 高风险动作。
...
- 当前规划验证：至少运行 `pnpm tasks:doctor`、`pnpm docs:readiness`、`pnpm docs:completion`、`pnpm risk:preflight`、`pnpm governance:preflight`、`pnpm residuals:validate`、`git diff --check`。
- 回滚：规划阶段只回滚本版本文档与任务；实现阶段坚持 additive-first schema evolution，仅允许已批准的 Subject code 约束放宽，以及 DailyReview/CheckIn/PeriodicReportDecision 日期或周期唯一约束向 workspace 复合唯一的替换。优先回滚应用代码并保留新增表字段；一旦写入自定义科目、第二工作区或 workspace 复合唯一数据，不得直接回滚到 `v0.1.7`，必须回到同一 Release train 已验证的 compatibility floor（当前地基为 `v0.1.9`）。任何 DROP、历史数据修复、文件移动或生产 restore 都需另行确认。
...
| 设置 | `/settings/ai` | Web 全局 AI 开关、当前浏览器外部 Provider 开关、服务端配置状态和 payload 隐私边界 |
...
- 本节旧路径表属于历史规划记录，不代表当前路由契约。当前 canonical 路由只以 `docs/ux/site-navigation.md`、`apps/web/lib/navigation/canonical-routes.ts` 与 `apps/web/lib/navigation/app-navigation.ts` 为准；旧路径不提供兼容重定向并应返回 404。现有 API 是否保留由 API 契约单独决定，不因页面路由迁移自动删除。
...
- 全局快速创建支持任务、考纲节点、知识卡片、错题和资料；知识点创建留在知识工作台，画布中的快捷创建仍调用同一真实对象表单和服务。
...
### 11.4 外部 Provider 当前浏览器偏好

- 外部 Provider 在每个浏览器中默认关闭。偏好缺失、清除、关闭或值畸形时统一解析为 `externalProviderEnabled=false`；不能用 truthy、`true` 字符串、旧 Cookie 或 Provider 配置存在来推断开启。
- 偏好只通过鉴权 `GET|PATCH /api/ai/preferences` 读取和保存。PATCH body 只接受严格布尔字段 `externalProviderEnabled`，空字段、错误类型和未知字段返回 400；未登录 GET/PATCH 均返回 401。
- 服务端使用与 session Cookie 名隔离的 host-only Cookie，值只允许 `enabled|disabled`；属性固定为 `HttpOnly`、`SameSite=Strict`、`Path=/`、最长 180 天，并在生产环境启用 `Secure`。不设置 Domain，不写 Prisma，不包含 user id、Provider 地址、模型、密钥、prompt、正文、hash、历史、token、费用或 trace。
- `/settings/ai` 的顺序固定为“当前浏览器开关 -> Provider/Payload Binding 只读状态 -> payload 隐私 -> 保存 AI 设置”。开关变化先进入开启/关闭策略确认 Modal；保存中禁用关闭和重复提交，取消、Escape、成功或失败后焦点回到“保存 AI 设置”。保存失败关闭 Modal、保留未保存选择、保持服务端已保存策略不变并要求显式重试。
- 八条鉴权显式 POST 路径共享同一 gate：`discipline`、`daily-review`、`tomorrow-plan` 三条建议，`learning-tree`、`knowledge-card`、`plan`、`motivation` 四类草稿，以及 `stage-adjustment-drafts/ai` 一条长期阶段草稿。每条 route 先鉴权、从同一请求读取偏好，再把布尔值传给 Provider resolver；禁止 route 内硬编码 `allowExternalProvider: true`。
- 旧 `/api/stage-adjustment-drafts/ai` 兼容 URL 只能 re-export canonical `/api/simulation/stage-adjustment-drafts/ai` 的 POST handler，计入同一长期阶段功能路径，不得复制 handler、另读偏好或形成第二套 payload/gate。
- 真实外呼必须同时满足 Web 全局 AI 开关开启、当前浏览器偏好开启、`AI_ENABLED=true`、Provider 配置完整，以及各路径既有的最小 payload/schema/preview/限流条件。任一条件不满足都回退本地规则；服务端 `AI_ENABLED=false` 仍可立即硬关闭全部外呼。偏好和 Web 开关都不会绕过 `AI_PAYLOAD_BINDING_SECRET`、四类草稿发送前预览或长期阶段最小聚合字段。
- `/settings/ai` 不提供 `AI_API_KEY`、`AI_PAYLOAD_BINDING_SECRET`、Provider URL 或模型编辑；本范围不新增第 25 条 migration，不保存 prompt/raw response/history/token/cost/provider trace，不改变 AI allowed fields，也不授权真实生产 key smoke、Release、生产 apply 或 residual 关闭。
- 回滚顺序固定为先设置 `AI_ENABLED=false`，再回滚 UI/API/route gate 并清除偏好 Cookie；回滚前不能只依赖新版本 Cookie，因为旧版本曾采用“鉴权显式 POST 即可外呼”的历史行为。
...
- `GET/PATCH /api/ai/preferences`：读取和保存 11.4 的当前浏览器外部 Provider 偏好；必须鉴权、严格布尔校验并使用安全 Cookie，缺失/畸形值 fail closed，不读写 Provider key 或数据库。
...
- 原 `/api/ai/**` 继续保持显式触发和最小上下文；三条建议、四类草稿和长期阶段草稿统一受 11.4 当前浏览器偏好 gate 约束，新草稿 API 不扩大附件、动机全文、完整复盘或后台外呼权限。
...
1. 地基 Release 1：下一可用 patch，OPS-006。**当前状态（2026-07-21）**：已由签名 Release `v0.1.9` 与生产 apply 满足；S2 复核通过后不再另开 OPS-006 patch。
2. 地基 Release 2：下一可用 patch，OPS-007。**当前状态（2026-07-21）**：已由签名 Release `v0.1.9` 与生产 apply 满足；S2 复核通过后不再另开 OPS-007 patch。
3. 完整产品 Release：下一可用 minor；先验证 compatibility floor，再由 migration runner 顺序应用八个产品 migrations，一次切换完整 Web。**仍为 Batch 11 / S5 唯一产品发布动作。**

- `v1.1` 是 workflow 阶段名，具体 tag 在各自 Release train 中根据当时版本确定。
- 完整产品 Release 不按工作台分批暴露，不增加生产功能开关或半成品双入口。
- 失败时优先回滚到 compatibility floor 应用镜像并保留 schema；数据库/上传目录 restore、DROP 或数据修复另行确认。新 custom subject、workspace composite unique 或多工作区根记录写入后，不允许直接回滚到 `v0.1.7`（当前 compatibility floor 起点为 `v0.1.9`）。
- 每次生产 backup、migration deploy、签名 Release、updater apply、smoke 和回滚目标分别确认、分别留证。
- 若 S2 复核发现 OPS-006/007 任一级 gate 证据不足，必须回退为本节原文的独立 patch Release 路径，不得带缺口进入 Batch 3。

### 14.5 实施确认与准入矩阵

| Gate | 进入时点 | 明确授权范围 | 必须证据 | 明确不授权 |
|---|---|---|---|---|
| `OPS-006 local implementation confirmed` | 已于 2026-07-18 达到 `local_verified` | 本地 additive active-session unique、task/session CAS、结束副作用单次提交、同日 CheckIn 锁和 409 映射 | V2 source binding、隔离 PostgreSQL migration/apply/repeat/dirty-preimage、并发 start/pause/resume/end/task/simulation/reorder/CheckIn fixture、doctor before/after | 生产 migration、历史 session 修复/合并/删除、Release apply、residual 关闭 |
...
1. Batch 0（S1）：同步产品、架构、模块、UX、安全、验证、任务和本 workflow；登记 `AF-RISK-DATA-001`；落高风险确认包骨架；消除当前恢复模式、旧路由和完成状态表述冲突。
2. Batch 1（S2）：原「独立确认、实施、验证并发布 OPS-006」。**现缩减为**：用 `v0.1.9` 证据复核四级 gate；仅在缺项时补证据或回退独立 patch。
3. Batch 2（S2）：原「独立确认、实施、验证并发布 OPS-007」。**现缩减为**：同上复核路径。
4. Batch 3（S3）：完成新依赖准入；在分阶段 migration 确认包内实施 Migration 1-3、工作区/科目兼容、知识卡片关系、里程碑/依赖和 PlanInbox core 规则；只提供隔离环境 API/fixture，不开放用户可见生产入口。
5. Batch 4（S3）：实现 `AREAFORGE_LEARNING_TREE_V1` parser/exporter、模板、无业务写入 preview、node/card/resource/plan diff 和 legacy importer 兼容；同时实施并验证 Migration 4 的 StudyResource schema/domain constraints；只有隔离 preview 入口，不开放 confirm 或生产资料入口。
6. Batch 5（S3）：在 Migration 4 已验证且达到 `OPS-007 local implementation confirmed + matching signed patch evidence` 后，实现 FILE/LINK 资料、Markdown/ZIP storage policy 和重复处理；随后实施 Migration 5，开放隔离环境 node/card/resource/plan 原子 confirm、导入历史与一次性导出。任何 resource directive 未满足 gate 时必须阻塞，不能静默忽略。
7. Batch 6（S3）：实施 Migration 6、统一复习、CheckIn v2、恢复、桥接和 PlanInbox 原子转换，完成核心服务/API 和 legacy fixture 验证。
8. Batch 7（S4）：建立受保护 App Shell、稳定路由、首次工作区设置、今日行动中心、科目快捷计时、全屏专注和快速复习；仅开放今日、任务、专注、快速复习和基础设置入口。
9. Batch 8（S4）：一次性实施 Migration 7 的 layout、motivation/reminder 和 notification schema；在隔离验收环境开放画布、考纲、卡片、错题、资料、导入和统一复习页面，以及跨设备布局、分层查询和画布快捷创建；动机/通知/AI 入口继续隐藏。
10. Batch 9（S4）：在既有 Migration 7 schema 上实现动机内容库、恢复动作、浏览器前台通知、通知偏好和经独立确认的四类显式 AI 草稿；开放动机、通知和上下文 AI 入口；同步 `.env.example` 与 `packages/config` 的 `AI_PAYLOAD_BINDING_SECRET` 服务端配置和客户端禁泄漏检查。
11. Batch 10（S4）：实施 Migration 8 的结构化模拟失分与 legacy totals 兼容；开放七天看板、模拟、报告和阶段入口，并完成资料、导入、画布、计划、复盘和阶段之间的跨页面完整闭环，以及桌面/移动、可访问性、错误恢复、观测信号和旧路由/API 兼容。
12. Batch 11（S5）：先形成本地完成记录与完整验证候选，通过 complete minor Release admission 后创建签名 minor Release；生产 backup、migration deploy、updater apply、smoke 与回滚目标仍分别确认。
...
- AI 偏好 GET/PATCH 未登录 401；空字段、错误类型和未知字段 400；缺失、清除、关闭或畸形 Cookie 均解析为关闭。
- 八条鉴权 AI POST route 都读取同一请求偏好且不存在 `allowExternalProvider: true` 旁路；偏好关闭优先于注入/配置 Provider，`AI_ENABLED=false` 或配置缺失继续 fallback。
- 偏好保存网络、401、4xx/5xx 或畸形响应失败时，已保存策略不变、Modal 关闭、错误可见且不自动重放。
...
- 今日页展示行动、可选证据和每日复盘三阶段进度，支持从最近收口下一动作断点续学，并允许对当前推荐记录“适合/换一项”反馈。
...
- 快速复习确认后停留结果页，由用户点击“下一项”；自由学习、专项复测和模拟完成后重新读取今日推荐，有候选时直接“继续下一项”；Inbox 转换、复盘、报告和阶段确认后停留原工作台并提供后续入口。
- 路线页按自然周维护每科显式投入预算并对比真实 session；趋势风险经服务端重算后可显式加入 Inbox；当前设备可配置收口字段提示和可选区展开方式，但不预填事实。
...
- `/settings/ai` 开启与关闭均经确认 Modal；刷新保持当前浏览器已保存偏好，清除/畸形 Cookie 恢复关闭；Escape、取消和保存失败后焦点恢复，桌面与 390px 移动端无横向溢出。
...
- `AF-RISK-SC-002` 与 `AF-RISK-SC-004` 的历史基线均为 `closed-evidence`；每次新 Release 前仍必须按目标 commit 重采 SC-002，并取得 fresh main protection readback / controlled PR。匹配 commit 的 CI、签名资产和 main protection readback 缺一不可。
...
- AI 外呼偏好证据：当前浏览器默认/缺失/畸形 fail closed，安全 Cookie 属性与严格 PATCH schema 通过；开启/关闭确认、刷新保持、失败可见和焦点恢复通过；三条建议、四类草稿和长期阶段草稿共八条鉴权 POST route 共享 gate，`AI_ENABLED=false` 时偏好开启也不外呼。
...
- 结构化浏览器证据：固定 9 条 journey × desktop/mobile 共 18 条，mutation 只能来自页面 UI，before/after oracle 只能 GET；独立无障碍固定 24 项并逐项绑定 observation JSON。两份顶层 JSON 必须由同一 production-build runtime/source commit/version/hash 采集，分别进入 Product Experience 与独立无障碍记录，再由 v1.1 admission 直接复验 schema、文件 hash、截图/artifact hash 和最终重读。
...
- 发布证据：若进入线上，记录新签名 Release、匹配 commit CI/供应链证据、main protection readback、GHCR digest、SBOM/provenance、备份、migration、health、登录态 smoke、update-agent、compatibility floor 回滚目标和 residual IDs；SC-002 未按目标 commit 刷新或 SC-004 未满足时不得进入 Release。
...
- 八个 product migrations 与其后四个 corrective/additive migrations 已有匹配确认范围，并在空库和 legacy fixture 一次性 PostgreSQL 16 中按完整 24 条仓库顺序验证；Subject code、DailyReview/CheckIn/PeriodicReportDecision 日期/周期复合唯一、workspace 接管、模拟状态回填、导入原子性、依赖并发和 OPS-006/007 专项 fixture 均通过。
...
- 2026-07-26 的浏览器目录来自 Next.js development runtime，含 Dev Tools/HMR、请求失败和 hydration mismatch；且没有完整九条旅程与独立无障碍记录，因此旧 UX PASS 不进入当前完成证据。
- 2026-07-22 compatibility floor 记录只绑定 20 条 migration 和旧候选 commit；当前仓库有 24 条，新增四条尚需扩展高风险确认与新鲜 12/15/24 manifest、ledger、floor probe、repeat deploy 证据。
- 旧 completion record 使用非法 result/fingerprint 口径且已被后续源码改动漂移；默认 validator 通过前不得宣称本地完成。
- 当前新鲜通过项：`pnpm check`（Core 94/94、AI 24/24、Storage 26/26、Auth 12/12、Web lint、Prisma validate、production build）及 M6/Task 隔离 runtime；其余专项和真实浏览器证据按本文件继续逐项复验。
- 本轮没有 commit、push、tag、GitHub Release、生产访问或写入、backup/restore、production migration、updater apply/rollback，也没有关闭或降级任何 residual；`AF-RISK-DATA-001` 继续保持 deferred-work，浏览器重采前保留 `AF-RISK-UX-001`。

## 2026-07-29 AI 外部 Provider 偏好专项状态

- 用户已确认 11.4 的窄范围实施；当前隔离候选已增加安全偏好 Cookie、鉴权 GET/PATCH 设置 API、`/settings/ai` 开关/保存/策略确认，以及八条既有鉴权 POST route 的共同外呼 gate。
- 专项静态/单元验收已覆盖默认与畸形 fail-closed、严格 PATCH schema、Cookie 属性、Provider resolver gate 顺序、八 route 枚举和客户端密钥禁区；本地 production-mode API 验收覆盖未登录 401、开启/关闭回读和 `AI_ENABLED=false` 时 `externalCall=false`。
- 本地 production-mode 浏览器验收覆盖开启/关闭 Modal、刷新保持、Escape 焦点恢复、桌面和 `390x844` 移动端无横向溢出；验证环境未配置真实 Provider，未执行真实 key smoke。
- 该专项只证明隔离候选中的外呼偏好能力，不能提升整个 v1.1 完成、Release admission、签名 Release、生产 apply 或 residual 状态；本轮未新增 migration、Provider key 编辑、payload 字段或 AI history/token/cost/trace 持久化。

## 2026-07-29 结构化浏览器证据门禁状态

- 已完成 production-build 浏览器证据 runner、固定 18 条 journey / 24 项无障碍 contract、独立 JSON validator、Product Experience journey 绑定，以及 complete minor Release admission 对 journey/accessibility 两份嵌套 JSON 的直接复验。
- admission 以同一 `sourceGitCommit`、`appVersion=1.1.0` 和 `productExperienceSourceHash` 强制 schema/hash/runtime identity；嵌套文件在 validator 期间或最终重读前变化会失败。evidence-only closeout 只允许固定两份 JSON、18 张 journey PNG、24 份 a11y observation JSON 与受限 runtime identity。
- `pnpm ops:v11:browser-evidence:selftest`、`pnpm experience:review:selftest`、`pnpm release:v11:admission:selftest`、`pnpm release:closeout:binding:selftest` 和质量脚本类型检查已通过；这些都是本地 contract/selftest，不是实际 9×desktop/mobile 旅程证据。
- 真实重采会对隔离 PostgreSQL 执行现有 migration（若尚未升级）、创建 18 个旅程合成账号与 1 个无障碍合成账号，并运行写入型页面 mutation；在取得独立精确确认前不得执行。它不涉及生产数据库、真实学习内容、Release、production apply 或 residual 关闭。

## 2026-07-31 Release 与发布后产品化修复状态

- `v1.1.0` annotated tag 和稳定 GitHub Release 已创建，Release 资产包含 manifest、SBOM、provenance、SHA256SUMS 与签名文件；公网 health 报告 production `1.1.0`、commit `4dbdb31a96498487af09aa7f90275bfc549448f3` 和 verified runtime identity。tag 本身没有 GPG signature，不能把资产签名扩写为“签名 tag”。
- 当前生产已经进入 v1.1 schema/product baseline，但首次设置、科目管理和核心工作台仍暴露明显产品化缺口；“路由可达/HTTP 200/旧证据通过”不再作为当前修复完成标准。
- 发布后产品化修复候选已修复接管 stable key 冲突、重复默认科目、无 ACTIVE workspace 引导，补齐科目与分组编辑/相邻排序/归档/恢复，并收敛 App Shell、今日、计划、知识、复盘、阶段和设置的信息架构；正式浏览器 runner 已同步当前标题、折叠表单、报告确认边界与双标签页布局冲突触发方式。既有完整 24 migration 隔离库上的 M1-M3 runtime 已按最新源码重跑通过，覆盖 archived-group 拒绝、相邻换位、边界 no-op、revision 和审计门禁；全量 `pnpm check` 已通过，九个核心入口在 `1440x900` / `390x844` 的当前源码快速验收为 18/18，且科目换位与归档分组编辑回退已实测。该快速验收和未提交诊断重跑不替代冻结 commit 后的正式 browser evidence runner/validator。后续当前工作树补齐科目归档排期暂停与恢复到当前学习日、今日进度、断点续学、推荐反馈、继续下一项、学习日结束、自然周预算、趋势风险入箱和当前设备收口模板，但尚未形成新 Release 或 production apply；既有历史 Release 证据不被这些当前工作树事实改写。
- 2026-07-31 已完成生产数据收口：数据库与上传目录备份校验通过；单事务将活动科目 `12 -> 7`，迁移新高等数学中的 1 次真实 session 和 1 张真实 note，保留并重命名 legacy 数学与四个 408 科目，删除 5 个重复科目；同时删除已盘点 `[AF_SMOKE]` 业务数据、22 条对应审计事件、2 条附件 metadata 和 2 个已备份合成文件。审计事件批准计数为 14、实际删除 22；额外 8 条为同批合成对象的 2 条 `attachment-created`、3 条 `session-start` 和 3 条 `session-end`，不涉及真实学习记录，但属于严格计数门禁偏差，双备份继续保留。清理后目标 ID、marker、审计事件均剩余 0，12 项只读完整性聚合与附件双向一致性检查通过。
- 22 个静态主要入口已在生产完成 2560px 桌面和 `390x844` 移动复验，22/22 可达、主内容存在、无错误页或横向溢出，console error/warning 为 0；`/settings/workspace` 显示最终 7 科且没有 `[AF_SMOKE]` 或旧 `408 ` 前缀。公网 health 与 Web 错误日志复核通过；由于本机缺少正式 production-readonly smoke 配置，该浏览器复核不替代正式 smoke record/validator。
- `v1.1.0` 已是不可变 Release 身份，不能覆盖发布；修复候选已提升为 `v1.1.1`，v1.1 专用 admission 与 Release workflow 只对 `v1.1.x` patch 系列启用并继续精确绑定 tag/version/commit。
- `v1.1.1` 修复 commit、current-bound browser/compatibility、SC-002/SC-004 与 Release 已完成；后续只剩独立 production apply 确认与相应生产证据。该步骤不自动关闭 residual，也不改变 `AREAFORGE_AUTO_APPLY=none`。
...
- 发布停止条件：任一高风险确认、数据生命周期确认或依赖准入缺失；目标 Release commit 的 SC-002 证据未刷新或 SC-004 远端证据缺失；migration 出现未批准的 destructive DDL 或旧唯一约束兼容未验证；临时库发现需要历史修复；parser/round-trip、导入原子性、依赖环、5,000 对象画布、无障碍、通知隐私或 AI 最小化失败；401 丢草稿或自动重放；409 强制覆盖；旧路由/API 失效；新页面提前暴露；签名、备份、migration runner、health、smoke 或 compatibility floor 回滚目标缺失。
