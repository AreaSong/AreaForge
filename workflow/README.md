...
- 后续持久 worker 首批已有兼容队列字段、退避/死信、租约代次、事务副作用、独立子进程恢复和受控排名通知处理器候选；完整 50 条 migration、15 组内核和 11 组通知域隔离 runtime 通过。通知域已获本地确认；排名重建、导出/删除处理器、共享/生产 migration 和 v2.0 门禁未完成。PR #57 / `b04ba98` 已合并，不作为待合并阻塞。
