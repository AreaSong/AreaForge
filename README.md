...
| 层级 | 当前事实 |
|---|---|
| 仓库状态 | `v1.1.0` 已发布并进入生产；当前 package version 已提升为 `1.1.1`，`codex/v1.1-learning-action-center` 分支继续准备发布后产品化修复候选，尚未 push、创建 `v1.1.1` Release 或 production apply |
...
| 当前收口 | 本地产品化修复已通过全量检查和桌面/移动验收；生产重复科目与 `[AF_SMOKE]` 数据已在备份和单事务下清理，22 个静态入口已完成桌面/移动只读复验。审计事件批准计数为 14、实际删除 22，额外 8 条均属于同批合成对象且偏差已记录；`v1.1.1` 还需冻结修复 commit 并重采 current-bound admission 证据 |
