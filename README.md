...
| A -> B 本地进度 | v1.3 动态个人版已合并 `main` 但未独立 Release；v1.4-v1.8 默认关闭的本地候选已通过总门禁与一次性隔离 runtime，commit `6566fab` 的 branch push CI run `34049447503` 已成功，六类身份 desktop/mobile 可见性与代表性写入矩阵已完成；v1.9 已形成纯规则与持久排名通知基础。当前 owner-required follow-up 仍待 Git 检查点；候选 migration 尚未 apply 到共享测试库或生产，仍缺完整失败变体、受保护 PR/合并、Release 和生产证据 |
