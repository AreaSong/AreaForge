...
`pnpm ops:long-term:gate` 是长期运营完成声明的严格 live evidence gate。它只读本地 redacted 证据文件和体验复核记录，要求 OPS-001、OPS-004、签名 Release 供应链和 14 天内 desktop/mobile 体验复核全部达到可人工关闭状态；缺证据时会失败，但不会联网、SSH、创建 Release、执行 updater、备份、恢复、migration 或修改 residual 台账。
...
- CI 供应链记录模板：`docs/development/ci-supply-chain-record-template.md`，SC-002 预检：`pnpm sc:sc-002:preflight`
...
- 维护窗口记录模板：`docs/development/maintenance-window-record-template.md`，生成入口：`pnpm maintenance:window:record`
- 事故记录模板：`docs/development/incident-record-template.md`
- 恢复演练记录模板：`docs/development/restore-drill-record-template.md`
- Update-agent redacted status 模板：`docs/development/update-agent-status-record-template.md`
- OPS-001 收口证据包模板：`docs/development/ops-001-closure-packet-template.md`
...
- 支持包预览：`docs/development/support-bundle-preview.md`
...
1. 按 `docs/development/long-term-operability-control-plane.md` 和 `docs/development/release-train.md` 判断是否进入 Release train，并同步 `docs/**`、`tasks/**`、`workflow/**` 和相关入口 README。
...
6. 若要声明“产品可长期运营”，先运行 `pnpm ops:long-term:gate`；它会在缺 OPS-001、OPS-004、签名 Release 供应链或新鲜体验证据时失败。
7. 通过 Web 版本中心提交受控更新请求，或由管理员执行服务器侧 updater。
