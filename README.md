...
| 区域 | 当前能力 |
|---|---|
| 今日作战台 | 双节点倒计时、今日任务、风险等级、连续打卡、任务欠账、阶段称号、恢复模式和行动型鞭策 |
| 学习执行 | 每日任务与债务流转、专注计时暂停/继续/恢复、结构化结束收口、有效学习判断、晚间复盘 |
| 知识沉淀 | 考纲进度树、作战地图、掌握条件与证据、复测记录、笔记、私有附件、错题与复习提醒 |
| 长期校准 | 近 7 天统计、长期风险、周审判、月复盘、全真模拟、阶段计划和需确认的调整草稿 |
| AI 建议 | OpenAI-compatible Provider、结构化输出校验、本地规则回退、最小化上下文和显式触发 |
| 私有运维 | 单管理员登录、PostgreSQL 持久化、备份恢复、签名 Release、服务器侧受控更新与回滚 |
...
| 层级 | 当前事实 |
|---|---|
| 线上基线 | `0.1.5`，已通过签名 GitHub Release `v0.1.5` 更新到 [forge.areasong.top](https://forge.areasong.top/)；健康检查返回 `0.1.5` |
| 功能完成度 | 文档定义的第一版必需项与第二阶段增强项当前均有实现和验证证据，附件、结构化学习状态、真实 AI、长期校准及生产发布/恢复/回滚闭环已完成 |
| 仓库演进 | 仓库在 `v0.1.5` 线上基线之后继续补齐长期运营控制面、CI 供应链证据、支持入口、维护记录与 repo-local Codex skills；这些仓库能力不能自动等同于线上已更新 |
| 更新策略 | `AREAFORGE_AUTO_APPLY=none`；Web 版本中心只提交受控请求，服务器侧 update-agent/updater 执行签名校验、备份、migration、切换、smoke 和回滚 |
| 残余证据 | 当前未完全关闭的是生产 extra/authenticated smoke 的新鲜证据、下一次签名 Release 的 SBOM/provenance、告警人工复核等长期运营证据，不是主功能缺失 |
...
不要从 README 猜测专项命令。按改动范围查阅 [验证矩阵](docs/development/validation-matrix.md)，发布前查阅 [Release train](docs/development/release-train.md)。
...
| 发布与长期运营 | [长期运营控制面](docs/development/long-term-operability-control-plane.md)、[Release train](docs/development/release-train.md)、[运营 readiness](docs/development/operational-readiness.md) |
...
仓库提供 repo-local Codex skills，用于发布、安全、文件存储、AI 治理、体验复核、SRE、供应链、文档同步和残余风险管理。源目录是 `.codex/skills-src/`，自动发现入口是 `.agents/skills/`；完整 owner 边界见 [.codex/skills-src/README.md](.codex/skills-src/README.md)。
