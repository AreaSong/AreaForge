...
| 层级 | 当前事实 |
|---|---|
| 仓库状态 | 最新稳定 GitHub Release 为 `v1.1.1`（commit `f995310`）；生产仍运行 `v1.1.0`（commit `4dbdb31`），`v1.1.1` production apply 尚未执行且需独立确认 |
...
| 当前收口 | 产品化修复已通过全量检查、桌面/移动验收、current-bound admission、受保护 PR 与稳定 `v1.1.1` Release；生产重复科目与 `[AF_SMOKE]` 数据已在备份和单事务下清理，22 个静态入口已完成桌面/移动只读复验。审计事件批准计数为 14、实际删除 22，额外 8 条均属于同批合成对象且偏差已记录；下一独立边界是 `v1.1.1` production apply |
